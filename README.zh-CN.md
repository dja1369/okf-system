...
### E2 — `title` 前面的 4 个字符（2026-07-26）

正文、文件名、路径**一个字节都没有改**。只是在答案 concept 的 frontmatter **`title`** 前面加了 4 个字符，N=400 时 `recall` 就从 **0.262 变成 0.533**（翻倍），N=24 时从 **0.400 变成 1.000**。同样的知识、同样的文件、同样的 bundle 规模。变的只有这一行在排序中的位置。

> **recall 不是正确率。** 合成 distractor 只给出路由性能的**上界**。
> **不要把 E2 的数字直接和 E1 比较，说成「变好／变差」** —— 预算（+11 B）和 filler 的生成方式都变了，两次实测是不同条件。可以比较的，是 **E2 内部这三种扰动条件之间**。
...
**R6 是为了证伪而设的判据，而这次证伪失败了。** R6 预注册的内容是「若改了 `title` 而 `recall` 不动，则排序支配这一诊断就是错的」，阈值 0.05。实测 spread 为 0.348~0.600，是**阈值的 7~12 倍**，证伪没有成立，诊断存活了下来。**在一个相关性信号为 0 的门里，这是预期中的结果，不是发现了 bug** —— 新的地方只在于，这个效应的大小第一次变成了数字。

**`cwdIndependent` 翻了过来。** 在 E1 里 N≥200 时全军覆没（0.000）的那 6 个 concept，只加一个前缀就变成 **0.967**。E1 的那个结果并不是「与仓库无关的知识吃亏」，而是**命名的函数**。

**R2、R3 又触发了**，因此按照预注册的处理规则，recall 的绝对值依然不作为任何策略结论的依据。

**在测量纪律上确实前进了一格。** E1 的 fixture 是在报告的 commit 里第一次出现的；E2 把 fixture 放进预注册的 commit，并由 smoke 通过 `git log --diff-filter=A` 强制严格不等号。

```sh
node test/gate-recall.mjs --e2 --perturb all   # 3 条件 × 5 级别 × 20 种子
```

这次运行记录为 300 个样本、**$0.00**、**26.6 秒**。这两个值都是**测量装置本身的性质**，不是对门的成本或速度的主张。

[E2 报告](docs/benchmarks/gate-recall-2026-07-26-e2.md) ·
[E2 预注册](docs/benchmarks/pre-registration-2026-07-26-e2.md)

<!-- okf-benchmark: 2026-07-26-e1 -->

### E1 — 门的 recall@cap，花费 $0.00（2026-07-26）

这次实测花费 **$0.00**，而且这不是声明，是运行时证明的：测量装置在启动 hook 前会把一个桩 `claude` 放在 `PATH` 最前面，而这个桩一次都没有被执行（`paidCallTrapTripped: false`）。4 个级别 × 20 个种子 = **80 个样本，6.2 秒**。

它只测一个数：`recall(N)` —— 当 bundle 里有 N 个 concept 时，事先冻结的 20 道题中，其答案 concept 存活进门实际注入的 index 的比例。

> **recall 不是正确率。** 这次测量只回答「门有没有把相关的那一行装进去」。「模型是否真的用了那一行」，不做付费调用就无法确认。合成 distractor 只给出路由性能的**上界**，因此真实使用中的 recall 比这更低。

| N | recall 均值 ± 标准差 | 样本 | min–max |
|---|---|---|---|
...
（把 `n=` 与 min–max 放在均值**同一行**，是 `test/bench-report.mjs` 立下、smoke 强制执行的规约。）

**R3 触发了**（违反随级别单调递减：N=100→200 处 +0.0025），**R2 也触发了**（`recall(24)` = 0.400 < 0.60）。按照预注册的处理规则，**recall 的绝对值不作为任何策略结论的依据。** 即便违规增量只有该级别种子间标准差的 1/16，这条规则依然成立 —— 预注册事先写明「增量小于标准差也不撤销触发」。上表照常发布，但不用它决定任何事。

**这次测量的实质发现是形状，不是水平。** 20 道题里有 9 道在所有级别存活率都是 0，3 道在所有级别都是 1.0，其余 8 道取中间值。按单元格算（20 道题 × 4 个级别 = 80），0 有 48 个、1 有 19 个、中间值有 13 个 —— recall 并不是二值的。中间值的题目里有 3 道反而随 N 变大而上升（q06: 0 → 0.60 → 0.65 → 0.70），这个上升正是 R3 的算术来源。门并不是每个类别各填一行就结束，而是**反复轮转类别直到预算耗尽**；每个类别只落到 1~3 行，真正的原因是单行很大 —— concept 行为 200~1,030 B，而 index 预算是 6,956 B，整体在 8~11 行就耗尽了。`references` 在任何级别都只取 1 行（N=200 时是 57 行里的 1 行），所以扎堆在那里的 8 个答案最多只能活 1 个。而决定这些位置由谁占据的不是相关性，而是**生成时的排序与行长度**，已确认的因素至少有 5 个。其中节内排序的键是**frontmatter 的 `title` 字典序，而不是文件名**（`lib/index-gen.mjs:315`；文件名只是 frontmatter 解析失败时的回退），而且**行的字节长度**同样会改变存活 —— 下一行若超出剩余预算，该类别就在那里停下（`lib/gate.mjs:122`）。现行的门里对 cwd、新鲜度、查询词的引用是 0 处。

**嵌套深度（轴 A-2）.** 固定 25 个 concept，内容完全相同，只把路径做深：

| 条件 | 注入的 concept 行 | 子域链接 |
|---|---:|---:|
| 扁平 | 28 | 0 |
| 2 层 | 27 | 0 |
| 3 层 | 26 | 0 |
...
每个条件只测了**一次**（n=1，没有重复种子），在这一次测量里每加深一层少一行。只有 4 个点，无法区分这个下降是否线性；深度 5 层以上没有测。以种下的 concept 为准，3 层是 25 → 23，**-8.0%**。原因是**字节压力**，不是链式展开失败 —— 路径每多一段，每一行都变长，最终把一行挤出预算。

```sh
node test/gate-recall.mjs     # 4 级别 × 20 种子，约 6 秒，零付费调用
node test/bench-nesting.mjs   # 嵌套深度轴
node test/smoke.mjs           # 回归防护
```

[E1 报告](docs/benchmarks/gate-recall-2026-07-26-e1.md) ·
[E1 预注册](docs/benchmarks/pre-registration-2026-07-26-e1.md)
