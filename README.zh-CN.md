...
**这次测量的实质发现是形状，不是水平。** 20 道题里有 8 道在所有级别存活率都是 0，3 道在所有级别都是 1.0 —— 这里的 recall 近乎二值，而不是一个连续的刻度。门按类别轮转、每个类别各填一行，所以答案扎堆 8 个的 `references` 类别，会有 7 个**结构性地**被挤掉 —— 与知识质量、题目难度都无关。而决定这 1~2 行由谁占据的唯一信号，是**文件名的字典序**：现行的门里对 cwd、新鲜度、查询词的引用是 0 处。
...
每加深一层正好线性少一行，没有崩塌点（3 层相对扁平为 -7.1%）。原因是**字节压力**，不是链式展开失败 —— 路径每多一段，每一行都变长，最终把一行挤出预算。
