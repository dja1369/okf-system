...
### ゲート recall@cap — 事前登録された3ラウンド、E1 → E3 (2026-07-26)

3ラウンドとも費用は **$0.00** で、宣言ではなく実行によって証明されている。ハーネスは `PATH` の
先頭にスタブ `claude` を置き、そのスタブが存在することを実測したうえで、スタブは一度も実行され
ない (`paidCallTrapInstalled: true`, `paidCallTrapTripped: false`)。

測定するのは `recall(N)` — バンドルに concept が N 個あるとき、凍結された20問のうち正解 concept
の行がゲートの実際に注入する index まで生き残る割合である。

> **recall は正答率ではない。**「ゲートが関連する行を載せたか」までしか答えない。モデルがその行を
> **使ったか**は有料呼び出しなしには検証できない。合成ディストラクタは**上限**しか与えないため、
> 実運用の recall はこれより低い。

**条件** — 3摂動 × 5レベル × 20シード = 300サンプル、28秒。正解 concept の frontmatter
**`title`** の前に4文字を付けるだけで、本文・ファイル名・パスは変更しない。

| N | `none` | `front` (`!!! `) **公開版** | `front` **引用符安全** | `back` (`힣힣 `) |
|---|---|---|---|---|
...
セルあたり n=20。E1 は `none` のみを11B小さい予算で実行し 0.400 / 0.277 / 0.245 / 0.248 を出した
— **別の条件**であり、上の表より良いとも悪いとも言えない。

**公開版の `front` 列は汚染されており、それを捕らえたのはその列自身のガードだった。** `!!!` は
YAML の**タグ指示子**である。引用符のない `title:` に付けると frontmatter が丸ごと壊れる —
type が失われ、リンクテキストがファイル名にフォールバックし、**description が消滅して**行が
約700Bから約30Bへ崩壊する。**凍結された20問のうち14問が引用符なしの title だ。** つまりその14問
では、この実験はソート位置ではなく**パース失敗**を測っていた。行が短ければ同じ予算にはるかに多く
の行が入る — N=24 で観測された `taken` 24 と行平均 263B がまさにそれである。引用符安全な接頭辞で
測り直すと `front` は**全レベル 0.400 の平坦**に崩れる。`none` と `back` は一桁も動かず、これは
修正が中立であることを確認すると同時に、`힣힣 ` が何も壊していなかったことを示す。

**生き残るものと崩れるもの。** ソートが生存を決めるという点は変わらない。N=400 での引用符安全な
spread は 0.400 − 0.185 = **0.215** で、反証しきい値 0.05 の **4.3倍**であり、`back` が recall を
0.262 から 0.185 へ押し下げるのは純粋な順序効果である。**関連性シグナルがゼロのシステムでは
これは予期される結果であってバグの発見ではない** — 新しいのはその大きさだ。しかし公開された大きさ
のうち3つは生き残らない。「4文字で recall が2倍」は 2.03倍 → **1.53倍**、「N=24 で 0.400 → 1.000」
は**変化なし**、E1 の `cwdIndependent` 0.000 → 0.967 は **0.000 → 0.333** になる。その代わりに
新しい事実が現れる。**ソートの前方に来ると recall は N にまったく依存しなくなる**(バンドルサイズ
17倍の範囲で 0.400 の平坦) — そのとき生存を決めるのは N ではなく `taken` だからである。

**生存条件は正確に `rank < taken` である** — カテゴリ内の title ソート順位が、そのカテゴリが実際に
載せた行数より小さいときにのみ生き残る。したがって recall は rank と `taken` 2つのベクトルの
**完全な**関数であり、近似なしに分解できる。N=24→50 では rank 成分が支配し(−0.15 ~ −0.41)、
N≥100 では ~0 に消える — 床効果である。正解 rank の平均(26.9)が `taken`(10.5)をはるかに超えると、
filler を増やしてもすでに外にある concept は変わらない。併記した留保: この分解は**因果ではなく
会計**であり、成分値は基準線に依存する。

**E3 が E2 に加えた訂正2つと、自身に加えた訂正1つ。** E2 は recall が N=100 から 400 まで
「単調に上がる」と書き、その原因を E3 に委ねた。事前登録した n=20 ではその上昇は**立たない** —
12の隣接ペアのうち `rising` は0個である。E3 の初版の見出しはそれゆえ上昇が「存在しない」と書いた
が、**それは誤りで**、敵対的な検出力チェックが捕らえた。n=60 では3ペアが `rising` であり
(最小 p=0.00027)、その3つすべてで `taken` 成分が動きの100%を担い rank 成分は正確に0である。上昇は
実在するが**実質的ではない**(中央値 CI = [0.000, 0.000])。E3 はさらに、「平坦」と「小さいが一貫
した動き」を混同していた E2 の `|Δ| ≤ 0.05` 規則を、正確な対応符号検定と分布によらない中央値
信頼区間に置き換え、方向と大きさを2つの値に分けて出す。

**旧 R3 はノイズに発火していた。** その文言は「単調減少違反 → **ハーネス欠陥** → 全結果破棄」だが、
実装は不確実性を扱わない平均比較だった。そのため ±0.005 のシードノイズが E1・E2 の両方で発火を
起こし、両ラウンドとも「発火したが何も破棄しない」という自己矛盾の状態で公開された。E3 はしきい値
を緩めず、検出対象を元の文言どおりに戻して整合性を直接測った。同じ300サンプルで旧 R3 は発火し、
新しい R3a は発火しない。

**ライブバンドルではソート偏りをまだ立証できない。** 読み取り専用で測り件数のみを出す —
title・説明・ファイル名・リンクは測定の外に出ず、`raw/` は開かない。ソートは
`title.toLowerCase()` を `<` で比較するので**ロケール照合ではなく UTF-16 コードユニット順**であり、
ASCII 先頭の title は常にハングル先頭より前に来る。ASCII 先頭がバンドルの 65.4% を占めゲートの
スロットの 70.6% を取るが、concept 26個では層化帰無モデルに対する超幾何正確検定が **p = 0.667**
である。結果とは言えない。小さいリフトを「ソートは無害だ」と読んでもいけない — ゲートは現在
候補の **65.4%** を載せており、すべてが載る場所ではソートは何も決めない(6カテゴリのうち2つが
自由度0)。カテゴリ別にはすでに分かれている: `decisions`・`projects` 1.000、`patterns` 0.500、
`references` **0.429**。初稿は積載率が下がれば効果が大きくなると書いたが、**ベンチマーク自身の
データがそれを反証した**ため、その主張は撤回した。

**スロットを取るのは関連性ではなく順序と行の長さである。** コードで確認された要因は5つ。
type セクション名の大文字小文字を区別するソートにより `# Subdirectories` が常に `# reference` より
前に来ること(`lib/index-gen.mjs:242`)、それによって入れ子の concept がカテゴリの前方に引き上げ
られること。セクション内では frontmatter **`title`** の辞書順であり、ファイル名ではない — ファイル名
はパース失敗時のフォールバックにすぎない(`:315`)。`status: deprecated` は後方へ下げられる
(`:245`)。カテゴリの巡回はディレクトリ名順(`:227`)。そして**行のバイト長** — 次の行が残り予算を
超えるとそのカテゴリはそこで止まる(`lib/gate.mjs:122`)。ゲートには cwd・新しさ・クエリへの参照が
1つもない。

**水準ではなく形が発見である。** 20問のうち9問は全レベルで0のまま、3問は1.0のまま生き残り、残る
8問がその間に入る — recall は二値ではない。ゲートは予算が尽きるまでラウンドロビンで埋める。
カテゴリが1〜3行で終わるのは1行が大きいからだ(1行 200〜1,030B に対し index 予算は約6,960B)。
そのため全体の取り込みは8〜11行で尽きる。`references` は全レベルでちょうど1行しか得ないので、
そこに集中した8つの正解のうち生き残れるのは最大1つである。

**入れ子の深さ (軸 A-2)。** concept 25個を固定し、内容は同一のまま、パスだけを深くした:

| 条件 | 注入された concept 行 | サブドメインリンク |
|---|---:|---:|
| フラット | 28 | 0 |
| 2段階 | 27 | 0 |
| 3段階 | 26 | 0 |
...
各条件は**1回ずつ**測定し(n=1、シード反復なし)、その1回の測定で深さ1段ごとに1行を失った。4点では
この減少が線形かどうかは判別できず、4段階より深いところは測っていない。植え込んだ concept を基準に
すると3段階は 25 → 23、**−8.0%** である。原因はチェーン走査の失敗ではなくバイト圧である — パスの
セグメントが1つ増えるたびにすべての行が長くなり、やがて1行が予算の外へ押し出される。

**R2 は全ラウンドで発火する**(`recall(24)` = 0.400 < 0.60)。事前登録した取り扱い規則により
**recall の絶対値は何も決定しない** — 表は公開するが方針を動かさない。

**測定規律と、改善した箇所。** E1 ではフィクスチャが**レポート**コミットで初めて git に入った —
しきい値は事前に固定されていたが、数字を実際に決めた材料はそうではなかった。E2 以降フィクスチャは
事前登録コミットに同梱され、スモークが `git log --diff-filter=A` で**厳密**不等号を強制する。E1 の
ファイル集合に向けると違反が3件出るので、この表明は実際の事故を承認するのではなく捕らえる。各ラウンド
は事前登録書を書いた時点ですでに知っていた値と、測定後に変更した算術を併せて公開する — E3 は
`0.25 − 0.20 = 0.04999…` なのに `0.20 − 0.15 = 0.05000…2` となり、同じ大きさの1問の移動が等価限界の
反対側に落ちるのを見てデルタを 1/20 の格子に量子化した。その修正はこのラウンド唯一の
`indeterminate` 判定を消し、**レポート自身の論拠に不利**だったが、その事実まで開示している。続いて
敵対的レビューが生存恒等式のガードがほぼ恒真であること(検査対象の関数をそのまま呼び直していた)を
示し、非循環な代替ガードは**初回の実行で発火して**上記の `front` 汚染を見つけた。推測で埋めずに抱えて
いる欠陥が1つある: 同じガードが摂動なしのサンプル100個のうち8個でも発火するが、原因はまだ特定できて
いない。

```sh
node test/gate-recall.mjs --e3 --perturb all   # 3条件 × 5レベル × 20シード、約28秒
node test/gate-recall.mjs --e3 --perturb all --quote-safe-perturb   # 訂正された接頭辞
node test/gate-title-distribution.mjs          # ライブバンドルの title 分布 (読み取り専用)
node test/gate-recall.mjs --e2 --perturb all   # E2
node test/gate-recall.mjs                      # E1
node test/bench-nesting.mjs                    # 入れ子の深さの軸
node test/smoke.mjs                            # 回帰ガード
```

[E3 レポート](docs/benchmarks/gate-recall-2026-07-26-e3.md) ·
[E3 事前登録書](docs/benchmarks/pre-registration-2026-07-26-e3.md) ·
[E2 レポート](docs/benchmarks/gate-recall-2026-07-26-e2.md) ·
[E2 事前登録書](docs/benchmarks/pre-registration-2026-07-26-e2.md) ·
[E1 レポート](docs/benchmarks/gate-recall-2026-07-26-e1.md) ·
[E1 事前登録書](docs/benchmarks/pre-registration-2026-07-26-e1.md)

<!-- okf-benchmark: 2026-07-27-efficiency -->

### ゲート効率 — インデックス形式はバイト分の価値があるか（軸 E, 2026-07-27）

E1〜E3 はいずれも OKF 自身の入力しか揺らしていない。だから「この形式は価値があるか」は問えなかった —
比較対象が存在しなかったからだ。軸 E がそれを立てる: **同じバンドル、同じバイト予算で、インデックス
戦略だけを 6 種に差し替える。** 費用は **$0.00**、今回も宣言ではなく PATH トラップで証明する。

質問はもう手書きしない。20 個の正解 concept それぞれについて、その **本文** の tf-idf 上位 8 語から
クエリを機械生成した — 本文はインデックスが決して載せない部分である。検索器は測定前に標準
パラメータを固定した BM25 で、その長さ正規化は長い行に罰点を与える。つまりこの選択は OKF に
**不利**な側に倒してある。シード数 40 は前ラウンドからの惰性ではなく、実行前の検出力計算で決めた。

**事前登録した 5 つの仮説のうち 2 つが支持され、3 つが反証された。**

| 仮説 | 判定 | 根拠 |
|---|---|---|
| タイトル+説明はカテゴリリンクのみに勝る | **支持** | 12/12 セルで okf 優位、すべて p<1e-4 |
| 説明はバイト分の価値がある | **反証** | 順序を揃えると説明を外した側が 12/12 セルで優位 |
| round-robin はオーバーヘッド分の価値がある | **条件付き反証** | 予算 2048 で −0.050、予算 9000 で +0.017〜+0.218 |
| 整列インデックスはランダムに勝る | 狭く支持 | okf が 7/12 で優位 — ただし N=26 の 3 セルは全敗 |
| パスだけでは足りない | **反証** | パスのみを載せる側が 8/12 セルで優位 |

1 行目は一度も測られたことのなかったものを閉じる。ライブバンドル自身のアーキテクチャ文書は、
2026-07-17 の「カテゴリ件数のみ」→「タイトル+説明」への変更の根拠を **事例 1 件** と記し、費用を
**n=3** で見積もっていた。今は数字がある。

**この形式は精度を買い、容量を売る。** OKF の行はいったん載れば、ほぼ必ず 1 位に選ばれる
（精度 0.93〜1.00）。ボトルネックは、既定の 9,000 バイトに concept 行が 12〜14 本しか入らないこと
だ。タイトルのみの形式は N=26 で 26 本すべてを載せ（精度 0.649）、パスのみの形式も 26 本すべてを
載せる（精度 0.350）。**説明は行バイトの約 82%** — 1 行 733 B、説明を外すと 133 B。

**round-robin の符号は予算で反転する。** 6 つのカテゴリがそれぞれ heading と省略マーカーを事前
控除するため、予算 2048 ではその固定費が 4 レベル全部で利得を食い潰す（−0.050）。出荷既定値の
9,000 では価値を出し、その利得はバンドルが大きいほど伸びる（N=200 で +0.218）。
**出荷既定値は自分の動作点では正しい** — そしてコードは予算に関わらず常に round-robin を使う。

> **これは「説明を捨てろ」という話ではない。** このラウンドは **見つける** ことを測り、**答える**
> ことは測っていない。ゲート規則 1 は「タイトルと説明が答えを含むなら Read せずその行を根拠に
> せよ」と約束しており、説明を外すとその経路が丸ごと死ぬ。説明がその 82% を取り返すかどうかは
> **有料軸であり、一度も走らせていない。** このラウンドが出したのは値札であって判決ではない。

**ライブバンドル、読み取り専用、件数とバイトのみ。** concept 26 個 / 108,431 B。ゲートは
**8,885 B — 予算の 98.7% — を使って 26 個中 14 個（53.8%）を見せている。** 圧縮比は 12.2×、注入
バイトの 71.6% が知識で 28.4% が構造。その構造のうち `log.md` の tail だけで 1,341 B、注入の
15.1% であり、heading と省略マーカーの合計の 2.6 倍にあたる。合成バンドルはこの 53.8% のカバレッジ
を **2.3 ポイント以内**で予測した — 合成設計に対する外部からの確認である。

**このラウンドは公開前に自分の欠陥を 1 件捕まえた。** 最初の登録実行では、パスのみ戦略の到達率が
12 セルすべてで 0.000 だった。そのまま読めば発見だが、実際はバグだった — 採点器が markdown リンク
構文からしかパスを抽出していなかった。修正すると、その仮説は支持から反証へ反転した。新規追加した
スモーク表明 9 件はそれぞれ変異試験にかけ、6 種の変異すべてが自分のガードを殺した。

**測れなかったこと、そのまま公開する**: BM25 は語彙の重なりであってモデルの判断ではない。バンドル
は合成なのでこの値は上限である。正解 concept の一覧は依然として私が選んだものだ（機械生成なのは
クエリだけ）。`paths` の成績は、このバンドルが韓国語本文 + 英字 slug であるという事実に依存する。
n=40 はシードの 80% に一貫する効果を 0.981 で捉えるが 70% なら 0.703 なので、ここでの「差がない」は
「立証できなかった」を意味する。ライブ標本は著者 1 人のバンドル 1 つだけ。トークン数はオフライン
トークナイザがないため測っていない。そして独立した敵対的レンズは走っていない — 検証は自己検証である。

```sh
node test/gate-efficiency.mjs                    # 4 レベル × 3 予算 × 40 シード、約 30 秒
node test/gate-efficiency.mjs --determinism-check
node test/gate-live-efficiency.mjs               # ライブバンドル、読み取り専用
```

[軸 E レポート](docs/benchmarks/gate-efficiency-2026-07-27.md) ·
[軸 E 事前登録](docs/benchmarks/pre-registration-2026-07-27-efficiency.md)

### 全区間の有料 run（v3, 2026-07-16）
