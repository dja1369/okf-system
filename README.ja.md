...
### E2 — `title` の前の 4 文字（2026-07-26）

本文・ファイル名・path は **1 バイトも変えていません。** 正解 concept の frontmatter **`title`** の前に 4 文字を足しただけで、N=400 で `recall` が **0.262 → 0.533** と倍になり、N=24 では **0.400 → 1.000** になります。同じ知識、同じファイル、同じ bundle の大きさ。変わったのは並び順の位置だけです。

> **recall は正答率ではありません。** 合成 distractor は router 性能の**上限しか**与えません。
> **E2 の数値を E1 と直接比べて「改善／悪化」と言ってはいけません** — 予算（+11 B）と filler の生成方法が変わったので、二つの run は異なる条件です。比較できるのは **E2 の中の 3 つの摂動条件どうし**です。
...
**R6 は反証するために作った基準で、その反証の試みが失敗しました。** R6 は「`title` を変えても `recall` が動かないなら並び順支配という診断は誤り」を閾値 0.05 で事前登録したものです。実測 spread は 0.348〜0.600 と**閾値の 7〜12 倍**で、反証は成立せず診断が生き残りました。**関連性の signal が 0 件の gate では、これは予期される結果であって bug の発見ではありません** — 新しいのは、その大きさが初めて数値になったことです。

**`cwdIndependent` が反転します。** E1 で N≥200 のとき全滅（0.000）していた concept 6 個が、接頭辞ひとつで **0.967** になります。E1 のあの結果は「repository に依存しない知識が不利」ではなく、**命名の関数**でした。

**R2・R3 はまた発火した**ので、事前登録した処理規則に従い、recall の絶対値は依然としていかなる policy の根拠にも使いません。

**測定の規律は一段進みました。** E1 では fixture が report の commit で初めて現れましたが、E2 では事前登録の commit に同梱され、smoke が `git log --diff-filter=A` で厳密不等号を強制します。

```sh
node test/gate-recall.mjs --e2 --perturb all   # 3 条件 × 5 レベル × 20 seed
```

この run は 300 サンプルで **$0.00**、**26.6 秒**と記録されます。どちらも**測定装置の性質**であり、gate の費用や速度についての主張ではありません。

[E2 report](docs/benchmarks/gate-recall-2026-07-26-e2.md) ·
[E2 事前登録](docs/benchmarks/pre-registration-2026-07-26-e2.md)

<!-- okf-benchmark: 2026-07-26-e1 -->

### E1 — gate の recall@cap、費用 $0.00（2026-07-26）

この run の費用は **$0.00** で、それは宣言ではなく実行で証明しました：harness は hook を起動する前に `PATH` の先頭に stub の `claude` を置きますが、その stub は一度も実行されませんでした（`paidCallTrapTripped: false`）。4 レベル × 20 seed = **80 サンプル、6.2 秒**。

測るのは数値ひとつです：`recall(N)` — bundle に concept が N 個あるとき、凍結した 20 問の正解 concept のうち、gate が実際に注入した index の中に生き残った割合。

> **recall は正答率ではありません。** この測定は「gate が関連する行を載せたか」までしか答えません。「model がその行を実際に使ったか」は有料呼び出しなしには確認できません。合成 distractor は router 性能の**上限しか**与えないので、実使用での recall はこれより低くなります。

| N | recall 平均 ± 標準偏差 | 標本 | min–max |
|---|---|---|---|
...
（`n=` と min–max を平均と**同じ行**に置くのは `test/bench-report.mjs` が定め smoke が強制する規約です。）

**R3 が発火し**（単調減少の違反：N=100→200 で +0.0025）、**R2 も発火しました**（`recall(24)` = 0.400 < 0.60）。事前登録した処理規則に従い、**recall の絶対値はいかなる policy の根拠にも使いません。** 違反 delta がその level の seed 間標準偏差の 1/16 しかなくても同じです — 「delta が標準偏差より小さくても発火を取り消さない」と事前登録が先に定めているからです。表は発行しますが、それで何かを決めることはありません。

**この測定の実質的な発見は水準ではなく形です。** 20 問のうち 9 問は全 level で生存率 0、3 問は全 level で 1.0 で、残る 8 問は中間値を取ります。cell 単位（20 問 × 4 level = 80）では 0 が 48、1 が 19、中間が 13 — recall は二値ではありません。中間値の問いのうち 3 問は N が大きくなるほど生存率が上がり（q06: 0 → 0.60 → 0.65 → 0.70）、この上昇が R3 の算術的な出所です。gate は category ごとに 1 行ずつ埋めて終わりではなく、**予算が尽きるまで category を繰り返し巡回**します。category あたり 1〜3 行で終わる本当の理由は 1 行が大きいからで、concept 行が 200〜1,030 B に対し index 予算は 6,956 B、全体で 8〜11 行で尽きます。`references` はどの level でも taken が 1 行（N=200 では 57 行中 1 行）なので、そこに集中した正解 8 個のうち最大 1 個しか残りません。その席を何が占めるかを決めるのは関連性ではなく**生成時の並び順と行の長さ**で、確認された要因だけで 5 つあります。なかでも section 内の並び順の key は**ファイル名ではなく frontmatter の `title` の辞書順**であり（`lib/index-gen.mjs:315`。ファイル名は frontmatter の parse に失敗したときの fallback にすぎません）、**行の byte 長**も生存を変えます — 次の行が残り予算を超えるとその category はそこで止まります（`lib/gate.mjs:122`）。現行 gate に cwd・新しさ・問い合わせ語への参照は 0 件です。

**入れ子の深さ（軸 A-2）.** concept 25 個を固定し、内容は完全に同一のまま path だけを深くしました：

| 条件 | 注入された concept 行 | 下位 domain link |
|---|---:|---:|
| 平坦 | 28 | 0 |
| 2 段 | 27 | 0 |
| 3 段 | 26 | 0 |
...
各条件は **1 回ずつの測定**（n=1、seed の反復なし）で、その 1 回の測定では深さ 1 段ごとに 1 行ずつ減りました。点が 4 つしかないため、この減少が線形かどうかは区別できません。深さ 5 段以上は測っていません。植えた concept を基準にすると 3 段は 25 → 23、**-8.0%** です。原因は chain 展開の失敗ではなく**byte 圧**です — path の segment が 1 つ増えるたびに全行が長くなり、1 行が予算の外へ押し出されます。

```sh
node test/gate-recall.mjs     # 4 レベル × 20 seed、約 6 秒、有料呼び出し 0
node test/bench-nesting.mjs   # 入れ子の深さの軸
node test/smoke.mjs           # 回帰 guard
```

[E1 report](docs/benchmarks/gate-recall-2026-07-26-e1.md) ·
[E1 事前登録](docs/benchmarks/pre-registration-2026-07-26-e1.md)
