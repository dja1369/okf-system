...
**この測定の実質的な発見は水準ではなく形です。** 20 問のうち 8 問は全 level で生存率 0、3 問は全 level で 1.0 — ここでの recall は連続的な目盛りではなく**ほぼ二値**です。gate は category ごとに 1 行ずつ round-robin で埋めるため、正解が 8 個集中した `references` category では 7 個が**構造的に**脱落します — 知識の質や設問の難易度とは無関係に。そしてその 1〜2 行を何が占めるかを決める唯一の signal は**ファイル名の辞書順**です：現行 gate に cwd・新しさ・問い合わせ語への参照は 0 件です。
...
深さ 1 段ごとにちょうど 1 行ずつ線形に減り、崩壊点はありません（平坦比で 3 段は -7.1%）。原因は chain 展開の失敗ではなく**byte 圧**です — path の segment が 1 つ増えるたびに全行が長くなり、1 行が予算の外へ押し出されます。
