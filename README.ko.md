...
측정 대상은 이전 세션이 확립한 사실 8개와 기억이 도움 되지 않는 대조 질문 1개입니다 — 아키텍처(SQLite / repository 패턴), 코딩 규칙(named export only), 과거 장애 수정(`busy_timeout=5000`), 응답 선호(한국어 / 간결), 파일·배포 정책(`src/config.mjs` / `npm run deploy:canary`), 무관한 산술 대조군(7 × 8 = 56). 조건 5개, 조건별 순서 교차 5회. C의 번들은 **실제** raw/ 수집 → 격리 batch ingest → SessionStart gate로 만들며 손으로 심은 concept은 없습니다. C가 목표 사실을 전부 포함하고 gate로 라우팅하며 D는 하나도 없을 때에만 preflight가 유료 실행을 허용합니다.
...
|---|---:|---:|---:|---:|---:|
| A — 기억 없음 | **0/5** | 12% | 27,246/27,518 | 13.82/18.17초 | $0.022218 |
| B_oracle (정답지) | 5/5 | 100% | 9,069/9,069 | 4.86/6.46초 | $0.008410 |
| B_realistic | 5/5 | 100% | 9,069/9,069 | 5.96/6.27초 | $0.008410 |
| **C — OKF 사용** | **5/5** | 100% | **10,395**/10,459 | 6.46/7.15초 | $0.011329 |
| D — 무관한 OKF | 0/5 | 0% | 20,602/21,662 | 14.50/21.15초 | $0.025879 |

행 뒤의 tool call이 숫자를 설명합니다. A는 4턴에 걸쳐 파일 2개를 읽고도 실패합니다. B는 정답이 이미 프롬프트에 있어 읽기 0회·1턴에 답합니다. **C도 읽기 0회·1턴에 답합니다** — gate index만으로 충분했습니다. D는 자기 gate가 애초에 갖지 않은 것을 찾아 2턴에 걸쳐 파일 1개를 읽습니다.

`p95`는 주의해서 읽으세요. n=5에서는 `ceil(0.95×5)−1`이 마지막 인덱스라 p95가 곧 **max**입니다 — 꼬리 통계가 아니라 cold cache 실행 하나입니다. 요청된 형식이 요구해서 싣는 것이지, 꼬리 통계여서가 아닙니다.
...
**"지식이 쌓일수록 OKF가 싸진다"는 거짓입니다.** 오히려 비싸지고, 대안보다 더 빨리 비싸집니다. 같은 벤치마크, 같은 번들에 무관한 concept 20개를 추가 — index에는 전부 들어갑니다(21줄, 9,000바이트 중 5,548, 잘린 것 없음):

| 조건 | 연속성 성공 | 준수율 p50 | token activity p50/p95 | wall p50/p95 | 비용 p50 |
|---|---:|---:|---:|---:|---:|
| A — 기억 없음 | 0/5 | 0% | 27,316/27,717 | 13.79/18.05초 | $0.022838 |
| B_oracle (정답지) | 5/5 | 100% | 9,070/9,085 | 5.33/6.78초 | $0.008410 |
| B_realistic | 5/5 | 100% | 10,406/10,406 | 5.72/9.62초 | $0.010134 |
| **C — OKF 사용** | **5/5** | 100% | **25,384**/25,773 | 11.75/13.15초 | $0.030721 |
| D — 무관한 OKF | 0/5 | 0% | 22,265/22,334 | 14.91/19.59초 | $0.037354 |

filler 0 실행과 비교하면 B_realistic은 **+1,337**(9,069 → 10,406) 늘었고 C는 **+14,989**(10,395 → 25,384) 늘었습니다. **C가 ~11배 빨리 나빠집니다** — concept 하나당 749 토큰 대 67 토큰. 둘 다 여전히 5/5를 맞히므로, 이것은 정확도 회귀가 아니라 순수한 비용 회귀입니다.

원인은 잘림이 아니라 신뢰입니다:

```
filler 0:   C 읽기=0  턴=1    index 줄만 보고 바로 답함
filler 20:  C 읽기=3  턴=4    다시 파일을 열러 감
```

무관한 concept 20개면 모델이 index 줄을 믿지 않고 파일로 검증하게 만들기에 충분했습니다 — gate 수정으로 없앤 바로 그 왕복이 되살아납니다. index는 그런 줄이 있다고만 말할 뿐, 그 줄이 *완전한* 답이라고는 말하지 않습니다. 주변 잡음이 커질수록 확인하는 쪽이 합리적인 선택이 됩니다. **이것이 진짜 천장이고 concept ~21개에서 옵니다 — 어떤 상한이 걸리기 한참 전입니다.**

잘림은 더 바깥의 두 번째 벽입니다. gate의 index는 Claude Code의 hook 10,000자 한도를 지키려 상한이 걸려 있고, 실제 한국어 concept 줄은 ~214바이트입니다:
...
~43개를 넘으면 index가 잘리고, 무엇이 살아남을지는 관련성도 최신성도 아닌 파일명이 정합니다. filler concept 50개 실행이 **preflight에서 실패하는** 이유가 정확히 이것입니다(`presentFacts: 8, routedFacts: 6, ready: false`) — `decisions/tech-stack.md`가 filler 뒤로 정렬되며 잘려나갔고 사실 두 개를 함께 데려갔습니다. 카테고리는 라운드로빈으로 배분해 굶는 카테고리는 없고 잘린 카테고리는 자기 `index.md`를 가리켜 나머지도 닿지만, 내려가려면 도구 왕복 — 같은 비용을 다시 냅니다.

두 벽 모두 튜닝 손잡이가 아닙니다. 첫 번째를 고치려면 index가 *어떤 줄이 완전한 답인지* 신호해서 모델이 파일을 열지 않고도 믿을 수 있어야 합니다. 그 작업은 되어 있지 않고, 될 때까지 OKF의 경제성은 concept이 하나 늘 때마다 나빠집니다.

축적 실행: [raw JSON](docs/benchmarks/raw/okf-live-2026-07-15T16-30-11-404Z.json). filler 50개 preflight 실패는 [preflight 감사 기록](docs/benchmarks/raw/okf-live-preflight-failed-2026-07-15T16-11-37-402Z.json)으로 보존합니다 — 의도적으로 남긴 부정 결과입니다.

harness는 결정 준수, 잘못된 가정, 추가 질문, tool call, 첫 유효 응답, API/wall time, `input_tokens`, `output_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`, CLI 비용도 기록하며 토큰 범주는 raw JSON에서 분리합니다. `tokenActivity`는 cache read를 output token과 1:1로 더하지만 cache read 과금은 ~50배 싸므로 **방어 가능한 열은 비용**입니다. n=5에서 `p95`는 산술적으로 항상 max(cold run 하나)입니다 — 위 표의 p95는 그 한계를 안고 읽으세요. CLI가 별도로 제공하지 않는 값(사용자 전용·gate 전용 토큰)은 추정하지 않고 `null`로 남깁니다.
...
2026-07-16 macOS arm64, Node `v26.4.0`에서 새로 측정한 중앙값과 범위입니다.

| 로컬 작업 | 중앙값 | 범위 |
|---|---:|---:|
| SessionStart gate 프로세스 | 57.2 ms | 56.9–58.1 ms |
| SessionEnd 트리거 프로세스 | 41.4 ms | 39.0–42.1 ms |
| 상태줄 프로세스 | 35.0 ms | 35.0–35.2 ms |
