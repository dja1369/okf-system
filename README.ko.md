...
concept은 하나도 없습니다.** 분량을 만드는 filler까지 포함해서입니다. 이는 들리는 것보다 중요합니다:
[축적](#축적-손으로-심은-filler로는-볼-수-없는-것)을 보세요.
...
`rfcs_policy`는 정직한 실패입니다: OKF는 2/5에 그쳤습니다. 문서 더미에 놓인 `N-2` 제안은 모델을
올바른 index 줄에서 끌어낼 만큼 강력한 함정입니다. 거기서 CLAUDE.md는 0/5였습니다.

### 축적: 손으로 심은 filler로는 볼 수 없는 것

같은 질문(`slim_buried`), 같은 harness, 실제 세션을 더 ingest해서 키운 번들.

| 번들의 concept 수 | gate 바이트 | OKF | CLAUDE.md | zero-base (평평한 레퍼런스) |
|---:|---:|---:|---:|---:|
...
비싸졌습니다($0.1279 → $0.2828).** 두 곡선은 갈라집니다.

이유는 두 번째 열에 보입니다. concept 15개와 35개 사이 — 지식이 2.3배 — 에서 gate는 **1바이트**
늘었습니다. batch가 중첩 도메인을 만들어 concept 14개를 index 한 줄로 접었기 때문입니다
(`- [slim](/references/slim/index.md): 하위 도메인 — concept 14개`). CLAUDE.md는 모든 프롬프트마다
모든 concept 본문을 실어 나르므로 선형으로 자랍니다. **gate는 그러지 않습니다.**

이것은 진짜 지식만이 만들어 낼 수 있었던 발견입니다. 이 벤치마크의 이전 실행은 손으로 filler를
심었습니다 — 손으로 쓴 concept 20개, 전부 평평하게, 전부 `decisions/`에. 그러면 index가 선형으로
자랄 수밖에 없고, 결론은 축적될수록 OKF의 경제성이 나빠진다는 것이었습니다. 실제 batch는 지식을
그런 식으로 쌓지 않습니다. 그 측정은 시스템이 아니라 fixture를 측정한 것이었습니다.

정확도는, 정직하게 말해서: 분량이 늘어도 좋아지지 않고 계속 들쭉날쭉합니다(2/5~5/5). n=5에서는
여기서 갈리는 게 없습니다.
