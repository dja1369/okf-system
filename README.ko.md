...
**이번 측정의 실질적 발견은 수준이 아니라 모양입니다.** 질문 20개 중 8개는 전 레벨에서 생존율이
0이고, 3개는 전 레벨에서 1.0입니다 — 여기서 recall은 연속적인 눈금이 아니라 **거의 이진적**입니다.
게이트는 카테고리마다 한 줄씩 round-robin으로 채우므로, 정답이 8개 몰린 `references` 카테고리는
7개가 **구조적으로** 탈락합니다 — 지식의 질이나 질문의 난이도와 무관하게. 그리고 그 1~2줄을
무엇이 차지할지 정하는 유일한 신호는 **파일명 사전순**입니다: 현행 게이트에 cwd·최신성·질의어
참조는 0건입니다.
...
깊이 1단계당 정확히 줄 하나씩 선형으로 줄고, 붕괴 지점은 없습니다(평면 대비 3단계 -7.1%). 원인은
사슬 펼치기 실패가 아니라 **바이트 압력**입니다 — 경로에 세그먼트가 하나 붙을 때마다 모든 줄이
길어져 결국 한 줄이 예산 밖으로 밀립니다. (25가 아니라 28인 것은 `ensureBootstrap`이 모든 조건에
같은 시드 concept를 심기 때문이며, 조건 간 비교에는 영향이 없습니다.)
