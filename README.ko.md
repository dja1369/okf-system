...
### E2 — `title` 앞의 4글자 (2026-07-26)

본문·파일명·경로는 **한 바이트도 바꾸지 않았습니다.** 정답 concept의 frontmatter **`title`** 앞에
4글자를 붙였을 뿐인데, N=400에서 `recall`이 **0.262 → 0.533**으로 두 배가 되고 N=24에서는
**0.400 → 1.000**이 됩니다. 같은 지식, 같은 파일, 같은 번들 크기입니다. 바뀐 것은 그 줄이 정렬에서
어디에 서는지뿐입니다.

> **recall은 정답률이 아닙니다.** 합성 distractor는 라우터 성능의 **상한만** 줍니다.
> **E2의 숫자를 E1과 직접 비교해 "개선/악화"라고 말하면 안 됩니다** — 예산(+11B)과 filler 생성
> 방식이 달라져 두 회차는 다른 조건입니다. 비교할 수 있는 것은 **E2 안의 세 섭동 조건끼리**입니다.
...
**R6은 반증하려고 만든 기준이고, 반증 시도가 실패했습니다.** R6은 "`title`을 바꿔도 `recall`이 안
움직이면 정렬 지배 진단은 틀렸다"를 임계 0.05로 사전등록한 것입니다. 실측 spread는 0.348~0.600으로
**임계의 7~12배**라, 반증 시도가 실패했고 진단이 살아남았습니다. **관련성 신호가 0건인 게이트에서
이것은 예상되는 결과이지 버그의 발견이 아닙니다** — 이번에 새로 나온 것은 그 크기가 숫자가 됐다는
점입니다.

**`cwdIndependent`가 뒤집힙니다.** E1에서 N≥200에 전멸(0.000)했던 concept 6개가 접두 하나로
**0.967**이 됩니다. E1의 그 결과는 "저장소 독립적 지식이 불리하다"가 아니라 **명명의 함수**였습니다.

**R2·R3이 또 발화했으므로**, 사전등록한 처리 규칙에 따라 recall의 절대값은 여전히 어떤 정책
근거로도 쓰지 않습니다.

**측정 규율 쪽에서는 한 칸 나아갔습니다.** E1은 픽스처가 리포트 커밋에서 처음 등장했지만, E2는
픽스처를 사전등록 커밋에 동봉했고 스모크가 `git log --diff-filter=A`로 엄격 부등호를 강제합니다.

```sh
node test/gate-recall.mjs --e2 --perturb all   # 3조건 × 5레벨 × 20시드
```

이 실행은 300샘플에 **$0.00**, **26.6초**로 기록됩니다. 두 값 모두 **측정 장치의 성질**이며, 게이트의
비용이나 속도에 대한 주장이 아닙니다.

[E2 보고서](docs/benchmarks/gate-recall-2026-07-26-e2.md) ·
[E2 사전 등록](docs/benchmarks/pre-registration-2026-07-26-e2.md)

<!-- okf-benchmark: 2026-07-26-e1 -->

### E1 — 게이트 recall@cap, 비용 $0.00 (2026-07-26)

이번 실행은 **$0.00**이 들었고, 그것은 선언이 아니라 실행으로 증명했습니다: 하니스는 훅을 띄우기
전에 `PATH` 맨 앞에 스텁 `claude`를 심고, 그 스텁은 한 번도 실행되지 않았습니다
(`paidCallTrapTripped: false`). 4레벨 × 20시드 = **80샘플, 6.2초**.

재는 것은 숫자 하나입니다: `recall(N)` — 번들에 concept가 N개 있을 때, 사전에 동결한 질문 20개의
정답 concept 중 게이트가 실제로 주입한 index 안에 살아남은 비율.

> **recall은 정답률이 아닙니다.** 이 측정은 "게이트가 관련 줄을 실었나"까지만 답합니다.
> "모델이 그 줄을 실제로 썼나"는 유료 호출 없이 확인할 수 없습니다. 합성 distractor는 라우터
> 성능의 **상한만** 주므로 실사용 recall은 이보다 낮습니다.

| N | recall 평균 ± 표준편차 | 표본 | min–max |
|---|---|---|---|
...
`n=`과 min–max를 평균과 **같은 행에** 싣는 것은 `test/bench-report.mjs`가 확립하고 스모크가
강제하는 규약입니다. 표본 2개짜리 중앙값을 곡선 위의 점처럼 보여 주는 사고를 다시는 못 내게
하려는 것입니다.

**R3이 발화했고**(단조 감소 위반: N=100→200에서 +0.0025), **R2도 발화했습니다**
(`recall(24)` = 0.400 < 0.60). 사전등록한 처리 규칙에 따라, **recall의 절대값은 어떤 정책 근거로도
쓰지 않습니다.** 위반 델타가 그 레벨의 시드 간 표준편차의 1/16에 불과해도 이 규칙은 그대로입니다 —
"델타가 표준편차보다 작아도 발화를 취소하지 않는다"를 사전등록서가 미리 못박았기 때문입니다. 위
표는 발행하되, 그 값으로 아무것도 결정하지 않습니다.

**이번 측정의 실질적 발견은 수준이 아니라 모양입니다.** 질문 20개 중 9개는 전 레벨에서 생존율이
0이고(q03·q07·q10·q13~q17·q20), 3개는 전 레벨에서 1.0이며(q02·q12·q18), 나머지 8개는 중간값을
가집니다. 셀 단위(20문항 × 4레벨 = 80)로는 0이 48개, 1이 19개, 중간이 13개입니다 — recall은
이진적이지 않습니다. 중간값 문항 중 q06·q11·q19는 오히려 N이 커질수록 생존율이 오르고
(q06: 0 → 0.60 → 0.65 → 0.70), 이 상승이 R3의 산술적 출처입니다. 게이트는 카테고리마다 한 줄씩
채우고 끝나는 것이 아니라 **예산이 마를 때까지 카테고리를 반복 순회**합니다. 카테고리당 1~3줄로
끝나는 진짜 이유는 줄 하나가 크기 때문입니다 — concept 줄이 200~1,030B인데 index 예산은 6,956B라
전체 taken이 8~11줄에서 소진됩니다. `references`는 어느 레벨에서도 taken이 1줄이라(N=200에서는
57줄 중 1줄) 거기 몰린 정답 8개 중 최대 1개만 살아남습니다. 그리고 그 자리를 무엇이 차지할지는
관련성이 아니라 **생성 시점의 정렬과 줄 길이**가 정하며, 확인된 요인만 최소 5개입니다: type
섹션명의 **대소문자 구분** 정렬(`# Subdirectories`가 `# reference`보다 항상 앞,
`lib/index-gen.mjs:242`), 섹션 안에서는 frontmatter **`title` 사전순**(`:315` — 파일명이 아닙니다.
파일명은 frontmatter 파싱 실패 시 fallback일 뿐입니다), `status: deprecated`의 섹션 내
후순위(`:245`), 디렉토리명 사전순인 카테고리 순회 순서(`:227`), 그리고 **줄 바이트 길이** — 다음
줄이 잔여 예산을 넘으면 그 카테고리는 거기서 멈추므로(`lib/gate.mjs:122`) description 길이가
생존을 바꿉니다. 현행 게이트에 cwd·최신성·질의어 참조는 0건입니다.

**중첩 깊이 (축 A-2).** concept 25개를 고정하고 내용은 완전히 동일하게 둔 채 경로만 깊게 만들었습니다:
...
각 조건은 **1회씩만 측정했고**(n=1, 시드 반복 없음), 그 한 번의 측정에서 깊이 1단계마다 줄이
하나씩 줄었습니다. 점이 4개뿐이라 이 감소가 선형인지는 구별하지 못하며, 깊이 5단계 이상은 재지
않았습니다. 심은 concept 기준으로 3단계는 25 → 23, **-8.0%**입니다. 원인은 사슬 펼치기 실패가
아니라 **바이트 압력**입니다 — 경로에 세그먼트가 하나 붙을 때마다 모든 줄이 길어져 결국 한 줄이
예산 밖으로 밀립니다. (25가 아니라 28인 것은 `ensureBootstrap`이 모든 조건에 같은 시드 concept를
심기 때문이며, 조건 간 비교에는 영향이 없습니다.)

```sh
node test/gate-recall.mjs     # 4레벨 × 20시드, 약 6초, 유료 호출 0
node test/bench-nesting.mjs   # 중첩 깊이 축
node test/smoke.mjs           # 회귀 가드 전체
```

[E1 보고서](docs/benchmarks/gate-recall-2026-07-26-e1.md) ·
[E1 사전 등록](docs/benchmarks/pre-registration-2026-07-26-e1.md)
