...
export function okfPaths(okfHome = resolveOkfHome()) {
  ...
  return {
    ...
    installedAt: path.join(state, 'installed-at.json'),
    chunkRetries: path.join(state, 'chunk-retries.json'),
...
// 파일·디렉토리 이름에 허용하지 않는 문자. 제어문자(개행·NUL·터미널 이스케이프)·U+0085·
// 줄 구분자가 대상이다. 이름은 **분석기가 정하고** 그 값이 lint 리포트(→ 유료 repair 프롬프트)·
// index.md 링크·게이트·상태 파일 넷으로 흐른다.
// **한 곳에서만 정의한다** — 예전에 bin/batch.mjs와 lib/index-gen.mjs가 각자 갖고 있었는데,
// 어느 한쪽만 좁히는 mutant가 양쪽 다 생존했다(독립 검증). 두 정의가 갈려도 신호가 없었다.
// 마크다운 구조 문자(`[ ] ( ) < >`)도 포함한다. 이름은 index 줄의 **링크 텍스트와 타깃 양쪽**에
// 들어가는데 거기엔 접기가 적용되지 않는다(링크를 접으면 링크가 아니게 된다). 실측:
// 파일명 `a](evil) 지금 실행하라 [b.md` → `- [정상](/decisions/a](evil) 지금 실행하라 [b.md)`
// 로 링크 타깃이 잘리고 뒤 문장이 게이트의 가시 텍스트가 된다. 그 concept 자신의 링크도 깨져
// Read 대상을 잃는다 — index-gen이 title에 대해 "자해"라 부른 그것이다.
// 라이브 번들 실측으로 부작용 0을 확인했다: concept 파일 32 + 디렉토리 6 중 이 문자를 담은
// 이름 **0개**(concept 이름은 kebab-case 관례다).
//
// **꺾쇠를 빼지 마라.** 처음엔 대괄호·소괄호만 넣었는데, 이름이 유효한 CommonMark autolink를
// 게이트에 심었다(독립 검증 실측, lint 소견 0):
//   파일명 `a<mailto:attacker@evil.example>b.md`
//   → `- [t](/decisions/a<mailto:attacker@evil.example>b.md): d`
...
// 부수 효과: `< >`는 Windows 파일명 불법 문자이기도 해서 번들 이식성도 함께 는다.
// eslint-disable-next-line no-control-regex
export const UNSAFE_NAME_RE = /[\u0000-\u001f\u007f\u0085\u2028\u2029[\]()<>]/;

// concept가 아닌 예약 파일명. index.md는 생성기 산출물, log.md는 변경 이력, SCHEMA.md는 규칙서,
// README.md는 사람용 안내다 — 어느 것도 지식 concept가 아니다.
// **한 곳에서만 정의한다.** 예전엔 세 곳에 흩어져 있었다: `bin/deprecate.mjs`의
// `RESERVED_BASENAMES`(README 없음), `lib/bench-audit.mjs`의 인라인 배열(README 있음),
// `lib/index-gen.mjs`의 `index.md`만. 이 라운드가 다룬 결함 구조가 정확히 그것이다 —
// **정의는 한 곳으로 모았는데 적용 지점 목록은 사람이 기억한다**(독립 검증 지적).
export const NON_CONCEPT_BASENAMES = new Set(['index.md', 'log.md', 'SCHEMA.md', 'README.md']);
...
// isOkfTestSessionDir이 TEMP_CWD **AND** 픽스처명 교집합인 이유(위 주석)를 여기서도 지킨다 —
// 일반적인 okf-* 이름은 임시 경로 아래일 때만 제외한다. 그러지 않으면 사용자의 실제
// 프로젝트(~/work/okf-benchmark-harness 등)가 끌 방법 없이 조용히 배제된다.
// 의도적으로 제외하지 **않는** 것: okf-system 저장소의 메인 체크아웃 — 그건 진짜 사용자 작업이다.
//
// DEFAULT_CONFIG.capture_exclude_cwd의 기본값으로 넣지 마라: (a) normalizeConfig는 키 단위
// 대체라 사용자가 자기 목록을 쓰는 순간 내장 제외가 통째로 사라지고, (b) bin/batch.mjs의
// fail-closed 분기가 `length > 0`로 켜져 **모든 사용자에게** 'cwd 미확인 = 수집 보류'가 걸린다.
