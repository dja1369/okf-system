...
export function frontmatterKeyLineRe(key, { flags = 'm' } = {}) {
  ...
  // 유효한 YAML 키 표기 셋을 전부 덮는다: 선행 공백(` status:`), 인용(`"status" :`), 콜론 앞 공백.
  // 그리고 **콜론 뒤에는 공백이나 줄 끝이 와야 한다** — 안 그러면 `status:: x`(실제 키는
  // `status:`)를 status로 오인해 남의 줄을 지운다(독립 검증 지적).
...
export function setFrontmatterStatus(content, value) {
  ...
  if (STATUS_LINE_RE.test(yamlText)) {
    nextYaml = value === null
      ? yamlText.replace(new RegExp(`${STATUS_LINE_RE.source}(\r?\n)?`, 'm'), () => '').replace(/(\r?\n)$/, '')
