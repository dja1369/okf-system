...
function resolveImport(lang, fromFileRel, spec, fileSet) {
  if (lang === 'python') return resolvePythonImport(fromFileRel, spec, fileSet);

  let p = spec;
  if (lang === 'rust') p = spec.replace(/^(crate|self|super)::/, '').replace(/::/g, '/');
  else if (lang === 'php') p = spec.replace(/\\/g, '/');

  if (p.startsWith('.') || p.startsWith('/')) return resolveRelativeImport(fromFileRel, p, fileSet);

  // 파일 기준 상대 -> 루트 기준 -> src 레이아웃 순으로 시도. C의 따옴표 include, Ruby의
  // require_relative, Rust의 mod가 첫 번째에서 잡힌다.
  return tryCandidates(path.posix.normalize(path.posix.join(path.posix.dirname(fromFileRel), p)), fileSet)
...
function extractFromFile(absPath, relPath, lang, fileSet) {
  ...
  try {
    ...
    if (fs.statSync(absPath).size > MAX_FILE_BYTES) {
      return { imports, decls, loc: -1, skipped: 'too-large' };
    }
    content = fs.readFileSync(absPath, 'utf8');
  } catch {
    return { imports, decls, loc: -1, skipped: 'unreadable' };
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const re of importPatterns) {
      ...
      if (m && m[1]) {
        const spec = m[1];
        const resolved = resolveImport(lang, relPath, spec, fileSet);
  ...
  return { imports, decls, loc: lines.length, skipped: null };
}
...
export function analyzeProject(projectRoot, opts = {}) {
  ...
  for (let i = 0; i < absFiles.length; i++) {
    ...
    const { imports, decls, loc, skipped } = lang
      ? extractFromFile(abs, rel, lang, fileSet)
      : { imports: [], decls: [], loc: -1, skipped: 'unsupported-language' };
