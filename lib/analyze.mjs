...
export function analyzeProject(projectRoot, opts = {}) {
  ...
  for (let i = 0; i < absFiles.length; i++) {
    ...
    const { imports, decls, relations, loc, skipped, namespace } = lang
      ? extractFromFile(abs, rel, lang, fileSet, ctx)
      : { imports: [], decls: [], relations: [], loc: -1, skipped: 'unsupported-language', namespace: null };
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'file') {
      ...
      if (!/분석 생략|구조 추출 미지원/.test(node.summary)) languageStats[lang].analyzedFiles++;
    } else {
