...
const DEFAULT_EXCLUDE_DIRS = new Set([
  ...
  '.ua', '.understand-anything', '.build', 'DerivedData', 'Pods', 'Carthage',
...
const IMPORT_PATTERNS = {
  ...
  c: [
    ...
    { re: /^\s*#\s*include\s+<([^>]+)>/, mode: 'c-angle' }, // 유일한 로컬 후보만 내부 헤더
  ],
...
const DECL_PATTERNS = {
  ...
  php: [
    { type: 'class', re: /^\s*(?:abstract\s+|final\s+|readonly\s+)*(?:class|interface|trait|enum)\s+(\w+)/ },
    { type: 'function', re: /^\s*function\s+&?\s*(\w+)\s*\(/ },
  ],
  c: [
    { type: 'class', re: /^\s*(?:class|struct|enum(?:\s+class)?|union)\s+(\w+)\b/ },
    { type: 'class', re: /^\s*namespace\s+(\w+)\b/ },
    { type: 'class', re: /^\s*typedef\b[^;{}]*\b(\w+)\s*;/ },
    // Function definitions require an opening body brace. Prototypes ending in `;` stay out.
    { type: 'function', re: /^\s*(?!if\b|for\b|while\b|switch\b|return\b)(?:[\w:<>~*&]+\s+)+([A-Za-z_]\w*)\s*\([^;{}]*\)\s*(?:const\s*)?(?:noexcept\s*)?\{/ },
  ],
  swift: [
    { type: 'protocol', re: /^\s*(?:(?:public|internal|private|fileprivate)\s+)*protocol\s+(\w+)/ },
    { type: 'class', re: /^\s*(?:(?:public|internal|private|fileprivate|open|final|indirect|nonisolated)\s+)*(?:class|struct|enum|actor)\s+(\w+)/ },
    { type: 'class', re: /^\s*(?:(?:public|internal|private|fileprivate)\s+)*typealias\s+(\w+)/ },
    { type: 'extension', re: /^\s*(?:(?:public|internal|private|fileprivate)\s+)*extension\s+([\w.]+)/ },
    { type: 'function', re: /^\s*(?:(?:public|internal|private|fileprivate|open|static|class|mutating|nonmutating|override|async)\s+)*func\s+(\w+)\s*[<(]/ },
  ],
};
...
DECL_PATTERNS.cpp = DECL_PATTERNS.c;

// 정규식 추출 전에 주석만 공백으로 바꾼다. 줄바꿈과 문자열 리터럴은 보존하므로 import 경로와
// 원래 line number는 유지하면서, 블록 주석 내부의 가짜 선언/import는 사라진다.
...
function parsePhpUses(line) {
  const match = /^\s*use\s+(?:(?:function|const)\s+)?(.+?)\s*;\s*$/.exec(line);
  if (!match) return [];
  const body = match[1].trim();
  const grouped = /^(.*?)\\\{(.+)\}$/.exec(body);
  const items = grouped ? grouped[2].split(',').map((v) => `${grouped[1]}\\${v.trim()}`) : body.split(',');
  return items.map((item) => item.trim().replace(/\s+as\s+\w+$/i, '').replace(/^\\+/, '')).filter(Boolean);
}
...
function tryCandidates(joined, fileSet) {
  ...
  for (const e of ['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.go', '.rs', '.rb', '.php', '.c', '.h', '.cc', '.cpp', '.hpp', '.cxx', '.swift']) candidates.push(joined + e);
...
function resolveImport(mode, lang, fromFileRel, spec, fileSet, ctx = {}) {
  ...
  if (mode === 'c-angle') {
    // A bare `<stdint.h>`-style name is overwhelmingly a system header even if the repo ships a
    // compatibility copy somewhere. Require an explicit path segment before considering it local.
    if (!spec.includes('/')) return null;
    const hits = [...fileSet].filter((candidate) => candidate === spec || candidate.endsWith(`/${spec}`));
  ...
  return tryCandidates(path.posix.normalize(path.posix.join(baseDir, p)), fileSet)
    || ((lang === 'c' || lang === 'cpp') && (
      tryCandidates(path.posix.normalize(p), fileSet)
      || tryCandidates(path.posix.join('include', p), fileSet)
      || tryCandidates(path.posix.join('src', p), fileSet)
    ))
...
function extractFromFile(absPath, relPath, lang, fileSet, ctx) {
  ...
  const decls = [];
  const relations = [];
  let namespace = null; // C# 또는 PHP 파일이 선언한 네임스페이스
  let content;
  try {
    ...
    if (fs.statSync(absPath).size > MAX_FILE_BYTES) {
      return { imports, decls, relations, loc: -1, skipped: 'too-large', namespace: null };
    }
    content = fs.readFileSync(absPath, 'utf8');
  } catch {
    return { imports, decls, relations, loc: -1, skipped: 'unreadable', namespace: null };
  }
  const lines = stripComments(content, lang).split('\n');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lang === 'php') {
      const nsm = /^\s*namespace\s+([\w\\]+)\s*[;{]/.exec(raw);
      if (nsm) namespace = nsm[1].replace(/^\\+|\\+$/g, '');
      const uses = parsePhpUses(raw);
      if (uses.length > 0) {
        for (const spec of uses) imports.push({ spec, resolved: null, external: true, phpSymbol: true });
        continue;
      }
    }
    ...
    for (const { type, re } of declPatterns) {
      const m = re.exec(raw);
      if (m && m[1]) {
        decls.push({ type, name: m[1], line: i + 1, topLevel: raw.length === raw.trimStart().length });
        break;
      }
    }

    if (lang === 'swift') {
      const declaration = /^\s*(?:(?:public|internal|private|fileprivate|open|final|indirect|nonisolated)\s+)*(class|struct|enum|protocol|actor)\s+(\w+)\s*(?::\s*([^\{]+))?/.exec(raw);
      if (declaration && declaration[3]) {
        const targets = declaration[3].split(',').map((v) => v.trim().replace(/\s+where\b.*$/, '').split(/[<&\s]/)[0]).filter(Boolean);
        relations.push({ sourceType: 'class', sourceName: declaration[2], targets, extensionTarget: null });
      }
      const extension = /^\s*(?:(?:public|internal|private|fileprivate)\s+)*extension\s+([\w.]+)\s*(?::\s*([^\{]+))?/.exec(raw);
      if (extension) {
        const targets = extension[2] ? extension[2].split(',').map((v) => v.trim().split(/[<&\s]/)[0]).filter(Boolean) : [];
        relations.push({ sourceType: 'extension', sourceName: extension[1], targets, extensionTarget: extension[1] });
      }
    }
  }
  return { imports, decls, relations, loc: lines.length, skipped: null, namespace };
}
...
export function analyzeProject(projectRoot, opts = {}) {
  ...
  try {
    ...
    throw new Error(`분석 대상 경로가 없습니다: ${projectRoot}`);
  }
  if (!rootStat.isDirectory()) throw new Error(`분석 대상이 디렉터리가 아닙니다: ${projectRoot}`);
  ...
  const phpUses = []; // [파일ID, fully-qualified symbol]
  const phpSymbolFiles = new Map(); // fully-qualified symbol -> declaring file
  const swiftRelations = []; // explicit inheritance/conformance/extension relations
  ...
  // 실제로 구조를 추출한 파일 집합. "files는 셌지만 analyzedFiles는 안 셌다"의 차이가
  // 곧 "측정하지 않은 파일 수"다.
  const analyzedRelPaths = new Set();
  for (let i = 0; i < absFiles.length; i++) {
    ...
    if (lang) fileLanguages.set(rel, lang);

    // 추출기 유무는 Object.hasOwn으로 본다 — `IMPORT_PATTERNS[lang] || []`는 프로토타입 체인을
    // 타서 lang이 'constructor'/'toString' 같은 값일 때 함수를 패턴 배열로 오인한다.
    // 추출기가 없는 언어(shell/markdown/json/yaml/toml)를 extractFromFile에 넘기면 loc는
    // 재지만 decls는 항상 0이 되어 "선언 0개"라는 **측정하지 않은 사실**을 보고하게 된다.
    const hasExtractor = Boolean(lang)
      && (Object.hasOwn(IMPORT_PATTERNS, lang) || Object.hasOwn(DECL_PATTERNS, lang));
    const { imports, decls, relations, loc, skipped, namespace } = hasExtractor
      ? extractFromFile(abs, rel, lang, fileSet, ctx)
      : { imports: [], decls: [], relations: [], loc: -1, skipped: lang ? 'no-extractor' : 'unsupported-language', namespace: null };
    ...
    else if (skipped === 'no-extractor') summary = `${lang} 파일 — 구조 추출기 없음(선언·import 미측정)`;
    ...
    if (!skipped) analyzedRelPaths.add(rel);
    nodes.push(fileNode);
    ...
    for (const d of decls.slice(0, 50)) { // 파일당 선언 상한 — 거대 파일이 그래프를 뒤덮는 것 방지
      ...
      nodes.push({
        ...
        scope: d.topLevel ? 'top-level' : 'nested',
    ...
    if (lang === 'php') {
      for (const declaration of decls) {
        const qualified = [namespace, declaration.name].filter(Boolean).join('\\');
        if (qualified) phpSymbolFiles.set(qualified.toLowerCase(), rel);
      }
    }
    if (lang === 'swift') {
      for (const relation of relations) swiftRelations.push({ rel, ...relation });
    }

    for (const imp of imports) {
      if (lang === 'php' && imp.phpSymbol) {
        phpUses.push([fileId, imp.spec]);
        continue;
      }
  ...
  // PHP `use` refers to symbols, not paths. Resolve only against declarations observed in this
  // repository, so Vendor\Package\User cannot accidentally point at Acme\Domain\User.
  for (const [fileId, symbol] of phpUses) {
    const target = phpSymbolFiles.get(symbol.replace(/^\\+/, '').toLowerCase());
    if (target) addEdge(fileId, `file:${target}`, 'imports', 0.7);
  }

  // Swift imports identify modules. File-to-file edges are created only for explicit type
  // relationships, which avoids turning ordinary identifier occurrences into a dense false graph.
  if (swiftRelations.length > 0) {
    const swiftDecls = new Map();
    for (const node of nodes) {
      if (node.type === 'file' || !node.tags?.includes('swift') || node.scope !== 'top-level') continue;
      if (!swiftDecls.has(node.name)) swiftDecls.set(node.name, []);
      swiftDecls.get(node.name).push(node);
    }
    for (const relation of swiftRelations) {
      const sources = swiftDecls.get(relation.sourceName) || [];
      const source = sources.find((node) => node.filePath === relation.rel && node.type === relation.sourceType)
        || sources.find((node) => node.filePath === relation.rel);
      if (!source) continue;
      if (relation.extensionTarget) {
        const target = (swiftDecls.get(relation.extensionTarget) || []).find((node) => node.type !== 'extension');
        if (target) addEdge(source.id, target.id, 'extends', 0.85);
      }
      for (const targetName of relation.targets) {
        const target = (swiftDecls.get(targetName) || []).find((node) => node.type !== 'extension');
        if (!target) continue;
        addEdge(source.id, target.id, target.type === 'protocol' ? 'conforms' : 'extends', 0.85);
      }
    }
  }
  ...
  const languageStats = {};
  for (const lang of languages) languageStats[lang] = { files: 0, analyzedFiles: 0, declarations: 0, internalEdges: 0 };
  for (const node of nodes) {
    const lang = node.tags?.find((tag) => languageStats[tag]);
    if (!lang) continue;
    if (node.type === 'file') {
      languageStats[lang].files++;
      // 예전엔 summary 문구를 정규식으로 되읽어 판정했다 — 요약 문구를 바꿀 때마다 조용히
      // 틀리는 판정이었다. 이제는 분석 시점에 세운 플래그를 그대로 쓴다.
      if (analyzedRelPaths.has(node.filePath)) languageStats[lang].analyzedFiles++;
    } else {
      languageStats[lang].declarations++;
    }
  }
  ...
  for (const edge of cleanEdges) {
    ...
    const sourceFile = source?.type === 'file' ? source.filePath : source?.filePath;
    const lang = sourceFile ? fileLanguages.get(sourceFile) : null;
    if (lang && languageStats[lang]) languageStats[lang].internalEdges++;
  }
  const primaryLanguages = Object.entries(languageStats)
    .filter(([, stats]) => stats.files > 0)
    .sort((a, b) => (b[1].declarations - a[1].declarations)
      || (b[1].internalEdges - a[1].internalEdges)
      || (b[1].files - a[1].files)
