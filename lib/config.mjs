...
export const DEFAULT_CONFIG = {
  ...
  claude_bin: '',
  node_bin: '',
  // 0 = 무제한(기본). 비용은 *보이게* 하되 기본 차단은 걸지 않는다.
  // 켤 때의 눈금: v4 체인 실측 중앙값 $0.4423/회이므로 2.0이면 대략 4~5회차.
  // best-effort 가드다 — .okf/last-batch.json의 당일 누계에 근거하므로 그 파일이 지워지거나
  // 손상되면 누계가 0에서 다시 시작하고, 배치가 SIGKILL로 죽으면 그 회차 지출은 안 잡힌다.
  batch_max_usd_per_day: 0,
  // 0 = 설치 이후 대화만 수집(기본). 설치 시각 이전으로 며칠까지 소급 수집할지.
  // SWEEP_LOOKBACK_DAYS(7일) 창은 그대로 남으므로 7을 넘겨도 추가 효과는 없다.
  sweep_backfill_days: 0,
};
...
const VALIDATORS = {
  ...
  // 정수 강제 안 함 — 센트 단위가 의미 있다.
  batch_max_usd_per_day: finiteNumber(0, 1000),
  sweep_backfill_days: finiteNumber(0, 30, true),
};
