...
export function ensureBootstrap(okfHome, log = () => {}) {
  ...
  if (!bundleExisted) {
    try {
      ...
      // err.message를 싣지 마라 — git 오류 메시지에는 절대경로가, YAML/파싱 오류에는 위반한
      // 원문(전사 파생 문자열)이 섞인다. bin/batch.mjs가 로그에 대해 지키는 계약과 같다.
      log(`git init failed: code=${safeErrorCode(err)}`);
  ...
  try {
    ...
    log(`bootstrap commit failed (git identity may need configuring): code=${safeErrorCode(err)}`);
  }
}
