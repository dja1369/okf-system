...
//
// 알려진 경계 사례: 판정은 transcript의 **mtime(마지막 활동 시각)** 이므로, 설치 이전에
// 시작됐지만 설치 이후까지 이어진 대화는 **통째로** 수집된다(앞부분이 설치 전이어도).
// 이것을 줄이려면 대화 내부를 시각으로 잘라야 하는데, 그건 캡처의 무손실 원칙을 깨고
// transcript 스키마에 의존하게 만든다. 사용자가 그 대화가 살아 있는 동안 설치했다는 점에서
// 동의 범위 안으로 본다 — 원치 않으면 capture_exclude_cwd로 그 작업 디렉토리를 제외한다.
