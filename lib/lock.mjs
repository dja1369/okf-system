...
//
// **나이 필터를 빼지 마라.** 나이를 안 보면 3자 경합이 lock.mjs의 존재 이유를 무너뜨린다
// (독립 검증이 코드 추적으로 지목했다):
//   (a) D가 방금 정상적으로 fresh 락을 쓴다.
//   (b) A가 (그 직전 판정 기준으로) rename해서 D의 fresh 락을 훔친다 — rename은 내용을 안 본다.
//   (c) A가 "훔쳤다"를 인지하고 linkSync로 되돌리려는 그 사이, **제3의 B**가 자기 acquireLock
//       진입에서 A의 claim을 지운다 → A의 linkSync가 ENOENT로 조용히 실패한다.
//   결과: D의 정당한 락이 디스크에서 완전히 사라진다(원본은 A의 rename이 소비, 사본은 B가 삭제).
//   D는 락을 쥐었다고 믿고 계속 쓰는데 또 다른 프로세스가 wx로 새 락을 잡아 **동시 쓰기**가 된다.
// 판별 신호는 **클레임 소유 PID의 생존**이다. 파일명이 `batch.lock.claim-<pid>-<uuid>`라
// 거기서 꺼낼 수 있고, 죽은 PID의 클레임만이 정의상 크래시 잔여물이다.
// 두 독립 검증이 같은 결함에 도달했고, 한쪽은 1ms 폴링으로 `.claim-*`를 12회 중 4번 실제로
// 목격했다 — 창은 서브마이크로초가 아니라 **밀리초 규모**다. "확률이 낮다"로 넘길 자리가 아니다.
//
// PID 재사용에 대비해 나이(mtime) 임계를 AND로 걸었다가 뺐다. 단락 평가 순서를 따라가 보면
// 그 조건은 아무 일도 하지 않는다: 재사용이란 "죽은 소유자의 PID를 산 프로세스가 물려받았다"이고
// 그러면 `pidAlive`가 true라 이미 건너뛴다 — 나이 검사에 도달조차 못 한다. 나이가 실제로
// 영향을 주는 유일한 경우는 "PID는 죽었고 크래시가 임계 이내"인데, 그건 **진짜 크래시 잔재를
// 회수하지 않는** 쪽이라 이 함수의 목적을 약화시킨다. mutation에서 그 줄만 지운 mutant가
// 생존한 것이 신호였다 — 테스트가 부족한 게 아니라 코드가 죽어 있었다.
function claimOwnerPid(name, prefix) {
  const rest = name.slice(prefix.length);
  const dash = rest.indexOf('-');
  const n = Number(dash > 0 ? rest.slice(0, dash) : rest);
...
function harvestAbandonedClaims(paths) {
  ...
  for (const name of safeReaddirSync(dir)) {
    ...
    // PID를 못 읽거나 살아있으면 남의 진행 중인 클레임일 수 있다 — 건드리지 않는다.
    // 회수를 한 회차 미루는 비용은 작고, 잘못 지우는 비용은 상호배제 위반이다.
    const pid = claimOwnerPid(name, prefix);
