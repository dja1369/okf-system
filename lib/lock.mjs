...
export function acquireLock(okfHome, holder, { onLog = () => {} } = {}) {
  ...
  let recoveredFromStaleLock = false;
  ...
  for (let attempt = 0; attempt < LOCK_ACQUIRE_MAX_ATTEMPTS; attempt++) {
    ...
    if (isLockStale(existing)) {
      ...
      // **지우기 직전에 되읽어 '내가 stale이라 판정한 바로 그 락'인지 확인한다.** 판정과 unlink
      // 사이에 다른 프로세스가 이미 회수하고 자기 락을 새로 썼다면, 그건 살아있는 남의 락이다.
      // 솔직히 적자면 이것은 경합을 **좁히는** 것이지 닫는 것이 아니다 — POSIX에 원자적
      // compare-and-delete가 없다. 남은 위험은 아래 wx 재획득의 token 되읽기가 한 번 더 거른다.
      const stillStale = readLock(paths.lock);
      if (JSON.stringify(stillStale) === JSON.stringify(existing)) tryUnlink(paths.lock);
