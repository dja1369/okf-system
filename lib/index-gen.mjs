...
function extractEntry(absPath, filename) {
  ...
  try {
    ...
    // `type`은 index의 **섹션 heading**이 된다(공식 규범). 값이 없으면 `Other`로 모인다 —
    // 공식 생성기(`bundle/index.py`)의 `grouped[typ or "Other"]`와 같은 처리다.
    const type = typeof data.type === 'string' && data.type.trim() !== ''
      ? foldToSingleLine(data.type) : 'Other';
    return { title, description, status: conceptStatus(data), type };
  } catch {
    return { title: fallbackTitle, description: undefined, status: 'stable', type: 'Other' };
  }
}
...
function buildRootIndex(rootIndexPath, summaries) {
  ...
  // 루트도 하위 디렉토리 목록이므로 `Subdirectories` 한 섹션이다(공식 4개 번들 루트 전부 그렇다).
  // 개수 표기는 공식에 없어 뺐다 — 설명 슬롯에는 그 디렉토리가 무엇인지를 쓴다.
  const rootGroup = new Map([['Subdirectories', summaries.map(({ dir }) => {
    const desc = DIR_DESCRIPTIONS[dir];
    return {
      sortKey: dir.toLowerCase(),
      line: desc ? `* [${dir}](${dir}/index.md) - ${desc}` : `* [${dir}](${dir}/index.md)`,
      isRetired: false,
    };
  })]]);
  ...
  // 프론트매터(`okf_version`)는 유지한다 — SPEC §12가 **루트 index.md에 한해** 허용하는
  // 유일한 예외다(공식 4개 번들은 v0.1이라 이 키를 쓰지 않지만 스펙이 명시적으로 허용한다).
  const text = `---\n${renderRootFrontmatter(block, okfVersion, changed)}\n---\n${renderSections(rootGroup)}`;
...
// 공식 생성기의 `_build_index_text`와 같은 출력을 만든다: 섹션 heading 알파벳 오름차순,
// 섹션 안은 링크 텍스트 대소문자 무시 오름차순, 섹션 사이 빈 줄 1개, 파일 끝 개행 1개.
function renderSections(grouped) {
  const sections = [];
  for (const type of [...grouped.keys()].sort()) {
    const items = grouped.get(type).sort((a, b) => {
      // 은퇴는 섹션 안에서 뒤로 — 우리 확장이고 스펙에 순서 규범이 없다.
...
function regenerateDir(okfHome, relParts) {
  ...
  // **공식 규범(`okf/src/reference_agent/bundle/index.py`)의 그룹핑을 그대로 따른다:**
  //   - 항목을 concept의 `type` 값으로 묶고 heading은 그 값 그대로 (`# Reference`).
  //     `type`이 없으면 `# Other`.
  //   - 하위 디렉토리는 언제나 리터럴 `Subdirectories` 그룹으로 간다.
  //   - 섹션 순서는 heading 알파벳 오름차순, 섹션 사이 빈 줄 1개.
  //   - 항목 순서는 링크 텍스트 대소문자 무시 오름차순.
  //   - 설명이 없으면 ` - ` 접미사 자체를 생략한다.
  // 실증: `stackoverflow/references/index.md`가 `# Reference` + `# Subdirectories` 두 섹션이고,
  // 이름이 다른 `joins/`·`metrics/`가 둘 다 `# Reference`다(안의 concept이 전부 그 type이라서).
  // 은퇴 표시는 우리 확장이다(S4). 스펙에 순서 규범이 없으므로 섹션 안에서 뒤로 보낸다 —
  // 게이트는 위치와 무관하게 접두사로 거른다.
  const grouped = new Map();
  const push = (type, sortKey, line, isRetired) => {
    if (!grouped.has(type)) grouped.set(type, []);
    grouped.get(type).push({ sortKey: sortKey.toLowerCase(), line, isRetired });
  };
  let deprecatedCount = 0;
  for (const name of files) {
    const { title, description, status, type } = extractEntry(path.join(dirPath, name), name);
    // **링크는 상대경로다**(공식 규범: `* [Customer Orders](orders.md) - …`).
    ...
    const retired = status === 'deprecated';
    if (retired) deprecatedCount++;
    // 마커는 링크 **바깥**에 — 안쪽이면 LINK_RE가 오염된다.
    push(type, title, retired ? `${DEPRECATED_PREFIX}${tail}` : `* ${tail}`, retired);
  }
  // 하위 도메인은 자기 index.md로 내려가는 링크로 싣는다 — 이게 점진적 공개다. 하위 concept를
  // 부모 index에 펼치면 깊은 번들에서 index 하나가 번들 전체 크기로 자란다.
  // 링크 텍스트는 **디렉토리 이름 그대로**이고(title이 아니다), 링크는 `dir/index.md`다 —
  // 공식 실물 74개 항목 중 `/`로 끝나는 링크는 0개다(SPEC §8 예시는 `subdir/`이지만 실물이 규범).
  // 개수 표기는 공식 어디에도 없어 뺐다.
  for (const sub of subdirs) {
    const desc = DIR_DESCRIPTIONS[sub];
    push('Subdirectories', sub, desc ? `* [${sub}](${sub}/index.md) - ${desc}` : `* [${sub}](${sub}/index.md)`, false);
  }

  writeAtomic(path.join(dirPath, 'index.md'), renderSections(grouped));
