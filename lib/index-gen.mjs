...
import { resolveOkfHome, okfPaths, SCAN_EXCLUDE_DIRS, UNSAFE_NAME_RE } from './paths.mjs';
...
function regenerateDir(okfHome, relParts) {
  ...
  const files = entries
    .filter((e) => e.isFile() && e.name.endsWith('.md') && e.name !== 'index.md' && !UNSAFE_NAME_RE.test(e.name))
    .map((e) => e.name)
    .sort();
