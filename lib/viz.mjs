...
<head>
<meta charset="utf-8">
<title>OKF 지식 그래프 — ${escapeHtml(path.basename(m.projectRoot || m.okfHome))}</title>
<style>
...
<div id="wrap">
  ...
  <div id="side">
    <h1>OKF 지식 그래프</h1>
    <div class="sub">${escapeHtml(m.okfHome)}<br>코드: ${escapeHtml(m.codeSource)}</div>
    <div class="stat">
      <div>concept <b>${m.okfCount}</b></div>
      <div>코드 <b>${m.codeCount}</b></div>
      <div>연결 <b>${m.crossCount}</b></div>
    </div>
    <input id="search" placeholder="이름·경로·태그 검색…">
    <div class="legend" id="legend"></div>
    <div id="detail"><div class="hint">노드를 클릭하면 상세가 보입니다. 드래그로 이동, 휠로 확대.</div></div>
  </div>
</div>
<script>
const DATA = ${json};
...
function radius(n) { return Math.min(4 + Math.sqrt(n.deg) * 2.2, 16); }
...
function draw() {
  ...
  for (const n of nodes) {
    ...
    if (view.k > 0.55 && (n.kind === 'okf' || n.deg > 4 || n === selected)) {
      ...
      ctx.font = (n.kind==='okf'?'600 ':'') + Math.max(9, 10/view.k) + 'px sans-serif';
...
for (const t of types) {
  ...
  b.onclick = () => { hidden.has(t) ? hidden.delete(t) : hidden.add(t); b.classList.toggle('off'); alpha = Math.max(alpha, 0.2); };
