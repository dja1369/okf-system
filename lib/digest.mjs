...
// 언랩이 여는 **새 유실 경로**: 인자 안에 사용자가 닫히지 않은 태그를 타이핑하면
// (`/plan 이렇게 <system-reminder>`) 언랩이 그것을 밖으로 내보내고, 뒤이은 블록 제거가
// 그것을 **나중에 오는 진짜 닫는 태그**와 짝지어 그 사이의 진짜 대화를 통째로 삼킨다.
// 삭제 방식에서는 인자 내용이 태그를 내보낼 수 없어 불가능했던 경로다(독립 검증 실측).
// 실패 양상이 조용한 지식 유실이라 이 프로젝트가 가장 싫어하는 형태다.
// 그래서 언랩할 내용은 **밖으로 나가기 전에** 정리한다: 완결된 블록은 제거하고, 남은 홀태그는
// 공백으로 만든다. 그러면 인자 내용이 바깥 텍스트와 짝지어질 수 없다.
const TAG_TOKEN_RE = /<\/?(?:command-name|command-message|command-args|local-command-stdout|local-command-caveat|system-reminder)>/g;

export function stripBoilerplate(text) {
  let out = String(text || '');
  // 함수 폼 — 인자에 `$&`·`$'` 같은 치환 패턴이 있어도 해석되지 않는다.
  out = out.replace(COMMAND_ARGS_RE, (_, inner) => {
    let s = inner;
    for (const re of BOILERPLATE_BLOCKS) s = s.replace(re, '');
    return s.replace(TAG_TOKEN_RE, ' ');
  });
...
export function digestFile(inputJsonlPath, outputDigestPath, capKb) {
  ...
  // digest는 **전사의 압축본**이다. 같은 디렉토리의 원본 `.jsonl`은 sweep이 0600으로 쓰는데
  // 파생물인 digest만 0644였다(독립 감사 실측). 이 저장소는 raw 파일명 하나 때문에
  // sessionLabel 해시까지 도입했다 — 그 내용물이 원본보다 느슨할 이유가 없다.
  writePrivateFile(outputDigestPath, digest);
  return {
