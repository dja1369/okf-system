...
import { parseFrontmatter, frontmatterRaw } from './frontmatter.mjs';
import { isPlainObject, generatedAt } from './trust.mjs';
...
function checkFrontmatterFidelity(content, data, relPath, warnings) {
  const raw = frontmatterRaw(content);
...
function checkNonReserved(relPath, hasFrontmatter, data, parseError, errors, warnings, isSchemaTemplate = false) {
  ...
  // v0.1 값이라 형식 강화가 곧 새 경고 폭증이고, §13.1은 레거시를 "지우지도 갱신하지도 말고
  // 그대로 둔다"이다.
...
export function runLint(okfHome) {
  ...
  for (const relPath of walkMdFiles(okfHome)) {
    ...
    if (!reserved) {
      ...
      checkFrontmatterFidelity(content, data, relPath, warnings);
    }
