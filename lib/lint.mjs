...
//  W14 분석기가 쓰면 안 되는 신뢰 필드(verified/sources/stale_after)의 존재
...
function checkFrontmatterFidelity(raw, data, relPath, warnings) {
  ...
  // W14. `templates/SCHEMA.md`와 `prompts/ingest.md`가 분석기에게 이 필드들을 **직접 쓰지 말라**고
  // 명시하는데(verified = 사람이 독립 확인했다는 신호, sources = digest가 URL을 보존하지 않아
  // 지금 채우면 지어내는 것), **코드 어디에도 검증·차단·스트리핑이 없다.**
  // `generated`는 fail-closed 스탬핑이, `status`는 청크당 은퇴 상한이 코드로 시행하는데
  // 이 셋만 순수 프롬프트 규범이다 — 이 저장소가 이 라운드 내내 찾아온
  // "정의(SCHEMA 표)는 있는데 적용 지점(코드)이 없다"와 같은 패턴이다(독립 리뷰 지적).
  //
  // **지금은 기능적 영향이 0이다**(lib/trust.mjs가 스스로 적듯 소비 코드가 없다). 문제는
  // 릴리스 3에서 신뢰 신호를 읽기 시작하는 순간, **그 이전에 이미 커밋된 위조 값이 소급 탐지
  // 없이 "확인됨"으로 읽힌다**는 것이다. 그래서 소비자가 생기기 전에 **쓰이는 시점의 신호**를
  // 남긴다 — 소급 탐지는 불가능하지만 동시 탐지는 지금 걸어야 가능하다.
  //
  // 차단이 아니라 경고인 이유: OKF 스펙 §11이 미지·추가 필드를 이유로 문서를 거부하지 말라고
  // 하고, 사람이나 외부 도구가 이 필드를 쓰는 것은 **정당하다**. 경고는 그 정당한 사용을
  // 막지 않으면서 분석기가 쓴 경우를 사용자에게 드러낸다.
  for (const field of ['verified', 'sources', 'stale_after']) {
    if (data[field] === undefined) continue;
    warnings.push({
      file: relPath,
      rule: 'W14',
      message: `${field} is present; SCHEMA.md reserves it for humans/external tools and no code consumes it yet`,
    });
  }
