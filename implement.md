...
```
[세션 사용]                      [배치 (detached claude -p, --safe-mode로 실행)]
...
> **OKF_HOME 안에서 세션을 시작한 경우 어떻게 되나** — 배치가 아닌 사용자가 실제로 `<OKF_HOME>` 디렉토리에서 작업하며 대화한 세션(예: 번들 내용을 직접 살펴보며 Claude와 대화)은 "한 세션의 모든 대화는 예외 없이 캡처"되어야 하므로 **정상 캡처된다** — cwd 기준 제외는 두지 않는다. 배치 자신의 `claude -p` 세션이 자기 자신을 재캡처하는 문제는 cwd가 아니라 §7-1의 두 가드(`--safe-mode` + `OKF_BATCH` env)로 막는다.
>
...
6.  digest들을 누적 300KB 청크로 분할. **각 청크를 순차 처리하며, 청크마다 6a~6e를 전부 완료한 뒤
    ...
    6a. ingest: prompt = prompts/ingest.md + "[OKF-BATCH]" 센티널 + 이번 청크의 digest 파일
        ...
        execFile('claude', ['-p', prompt,
          '--tools', 'Read,Glob,Grep,Write,Edit',           # 실측 완료: --allowedTools는 권한 프롬프트
          '--disallowedTools', 'Bash',                       # 생략 목록일 뿐 실제 도구 가용성을 제한하지
          '--settings', '{"hooks":{}}',                      # 않음 — --allowedTools에서 Bash를 빼도 모델이
          '--safe-mode',                                     # Bash를 호출하면 그대로 실행됨을 실측 확인.
          '--permission-mode', 'acceptEdits',                # --tools(가용 도구 집합 자체를 제한)로 교체 +
          '--max-turns', '80',                               # --disallowedTools Bash 병기해야 실제로 막힘(§9-4).
        ], {
          cwd: OKF_HOME, timeout: 15*60*1000,
          shell: process.platform === 'win32',              # claude.cmd 대응 (§2, §9)
          env: { ...process.env, OKF_BATCH: '1' }            # defense-in-depth. 1차 가드는 위 --safe-mode
        })                                                    # (아래 §7-1 참고 — CLAUDE_CONFIG_DIR 격리는
                                                               # 실측 결과 인증까지 함께 깨뜨려 폐기했다).
...
실제로 남기는 2개:
1. **`--safe-mode`** (1차 방어, 실측 후 확정 — 원래 계획했던 `CLAUDE_CONFIG_DIR` 격리에서 교체): 배치의 `claude -p` 호출에 `--safe-mode`를 지정해 훅·플러그인·MCP·커스텀 커맨드 전부를 세션 단위로 비활성화한다 — 로드되지 않으니 SessionEnd/SessionStart 자체가 발화하지 않아 구조적으로 루프가 성립하지 않는다. **`CLAUDE_CONFIG_DIR` 격리는 실제로 폐기했다**: 완전히 새 config 디렉토리를 주면 keychain/OAuth 세션까지 함께 격리되어 `claude -p`가 그 즉시 "Not logged in"으로 실패한다(실측 확인 — API 키(`ANTHROPIC_API_KEY`) 사용자만 우연히 동작하고, OAuth/구독 로그인 사용자는 배치가 원천적으로 동작하지 않는 치명적 결함이었다). `--safe-mode`는 "Auth, model selection, built-in tools, and permissions work normally"라고 명시되어 있고, 실측으로도 동일 세션에서 인증은 유지되면서(정상 응답) 플러그인 훅(이 플러그인 자신 포함)은 발화하지 않음(OKF_HOME 미생성)을 확인했다.
2. **`OKF_BATCH=1` env** (defense-in-depth): 1차 방어가 어떤 이유로든 불완전할 경우를 대비해, 혹시 우리 훅이 로드되어 실행되더라도 첫 줄에서 즉시 종료하도록.
→ `--settings '{"hooks":{}}'`는 1번의 보조 수단으로 병기(`--safe-mode`와 겹치지만 값싼 이중 방어). 1번·2번 모두 §9에서 실측 완료.
...
2. ~~`claude -p` 자식으로의 env 상속 및 CLAUDE_CONFIG_DIR 격리~~ **실측 완료, 설계 변경**: `CLAUDE_CONFIG_DIR` 격리는 훅 차단은 되지만 **keychain/OAuth 인증까지 함께 깨져서 배치가 "Not logged in"으로 즉시 실패**함을 실측으로 확인(§7-1 참고) — API 키 사용자만 우연히 동작했을 치명적 결함이라 폐기하고 `--safe-mode`로 교체, 인증 유지 + 훅 미발화 둘 다 실측 확인했다.
...
5. ~~`--settings '{"hooks":{}}'` 및 `CLAUDE_CONFIG_DIR` 격리 동작~~ **실측 완료** — 위 §9-2 참고. `--safe-mode`가 실제 CLI에서 스킬/플러그인/훅/MCP를 비활성화하면서 인증은 정상 동작함을 실제 로그인 세션으로 확인했다(플러그인을 `--plugin-dir`로 세션 한정 로드한 뒤, `--safe-mode` 유무에 따라 OKF_HOME 부트스트랩 여부가 갈리는 것으로 검증).
