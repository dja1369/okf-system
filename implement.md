...
6.  digest들을 누적 300KB 청크로 분할. **각 청크를 순차 처리하며, 청크마다 6a~6e를 전부 완료한 뒤
    ...
    6a. ingest: prompt = prompts/ingest.md + "[OKF-BATCH]" 센티널 + 이번 청크의 digest 파일
        ...
        execFile('claude', ['-p', prompt,
          '--allowedTools', 'Read,Glob,Grep,Write,Edit',   # 정확한 플래그명/의미(화이트리스트인지,
          '--permission-mode', 'acceptEdits',               # 단순 프롬프트 생략인지)는 구현 시
          '--max-turns', '80',                              # `claude --help`로 재확인 필수(§9) — Bash를
        ], {                                                 # 반드시 실행 불가능 상태로 만들어야 함
          ...
          env: { ...process.env, OKF_BATCH: '1',
                 CLAUDE_CONFIG_DIR: <배치 전용 격리 스크래치 디렉토리> }  # 사용자의 다른 훅/플러그인이
                                                               ...
                                                               # --settings '{"hooks":{}}'는 보조 수단으로
                                                               # 병기(둘 다 §9에서 실제 동작 실측 필요)
...
4. **정확한 도구 제한 플래그**: `--allowedTools`가 실제로 "화이트리스트"인지 "권한 프롬프트 생략 목록"인지 현재 CLI 버전에서 재확인(`claude --help`) — Bash를 반드시 실행 불가능하게 만드는 올바른 플래그(`--allowedTools`/`--tools`/`--disallowedTools` 등 후보)로 §5-5를 갱신.
