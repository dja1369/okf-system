...
for (let rep = 0; rep < runs; rep++) {
  for (const cell of cells) {
    ...
    records.push(sanitize({
      ...
      answerInGateIndex: cell.condition === 'okf'
        ? regexHit(cell.scenario, gate).hits >= Math.ceil(cell.scenario.answer_keywords.length * 0.5) : null,
      measurement: m,
      grade: {
...
const summary = Object.fromEntries(Object.entries(byCell).map(([k, c]) => {
  ...
  return [k, {
    ...
    answerInGateIndex: c.rows.filter((r) => r.answerInGateIndex === true).length,
...
const out = {
  ...
  metricDefinitions: {
    ...
    answerInGateIndex: '게이트 인덱스 텍스트만으로 정답 키워드의 과반이 노출된 셀 — 그런 런의 C 는 사실상 정답지 조건이다.',
  },
  summary, records,
};
