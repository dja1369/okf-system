...
for (const key of scenarioKeys) {
  ...
  for (const cond of ['zero_base', 'answer_sheet', 'okf', 'wrong_knowledge', 'claude_md']) {
    ...
    if (!c) continue;
    // 정답이 하나도 없으면 '정답런만' 비용은 존재하지 않는다. 그 칸을 비워두면 '0/5인데 쌌다'는
    // 오독이 생기므로, 전체 런 비용을 괄호로 명시한다 — 틀리는 데 쓴 돈도 쓴 돈이다.
    const costCell = cost(c) != null ? usd(cost(c))
      : (c.costUsdAll ? `(정답 없음 · 전체 ${usd(c.costUsdAll.p50)})` : 'n/a');
    lines.push(`| ${CONDITION_LABEL[cond]} | ${c.correct}/${c.runs} | ${costCell} | ${c.tokenActivity?.p50 ?? 'n/a'} | ${c.tokenActivityExCacheRead?.p50 ?? 'n/a'} | ${c.toolCalls?.p50 ?? 'n/a'} | ${c.turns?.p50 ?? 'n/a'} |`);
  }
...
const outPath = path.join(ROOT, 'docs', 'benchmarks', `okf-benchmark-2026-07-16.md`);
