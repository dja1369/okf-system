...
// 다만 지금의 검증 집합("대상이 이미 있다"·"원본이 없다")에서는 그 연쇄가 **성립하지 않는다** —
// 앞 항목의 목적지는 아직 없는 경로라 뒤 항목의 원본이 될 수 없기 때문이다. 실제로 이 함수를
// 순차 치환으로 바꾸는 돌연변이는 테스트를 통과한다. 검증 규칙이 완화되면 즉시 되살아나는
// 위험이라 단일 패스를 유지하되, 그 사실을 여기 적어 둔다(자기충족 테스트를 쓰지 않기 위해).
...
function main() {
  ...
  try {
    if (isDirty(okfHome)) {
      ...
      // 지금은 이 분기를 발동시킬 입력을 만들 수 없다: lint와 재작성이 같은 파일 집합을 보고
      // (둘 다 심볼릭 링크와 루트 SCAN_EXCLUDE_DIRS를 건너뛴다) 재작성이 그 안의 절대경로 링크를
      // 빠짐없이 고치기 때문이다. `after > before`를 지우는 돌연변이가 살아남는 이유다. 링크
      // 형식이 하나라도 늘면 곧바로 필요해지는 그물이라 남긴다.
