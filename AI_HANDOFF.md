...
배포 버전은 `0.2.2`. 브랜치 `bench/0-2-axis-a`에서 계획서 **I6**(게이트 recall@cap 측정 장치)를
구축·사전등록·발행했다. 측정은 전부 **$0**이고, 유료 0은 선언이 아니라 PATH 트랩으로 실행 중에
증명한다. `node test/smoke.mjs` **713 passed / 0 failed**.

`main`은 브랜치 보호(PR 필수)로 잠겨 있다 — 직접 push·merge·force-push는 하지 않는다.

**다음 작업자가 알아야 할 것 하나**: R3이 발화했으므로 이 회차의 recall 절대값으로 정책을
결정하면 안 된다. 사전등록서 §6이 그 처리 규칙을 미리 못박아 뒀다. 평탄화 가설을 검증하려면
**별도 사전등록**이 필요하다.
...
**E2 벤치마크 — 정렬 지배 가설 검정 (같은 브랜치, $0).**

E1의 적대적 검증이 남긴 결함 5건을 닫는 회차. 사전등록 `20d0b3b`, 리포트는 그 뒤 커밋.

- **핵심 결과**: 본문·파일명·경로를 한 바이트도 바꾸지 않고 정답 concept의 frontmatter
  `title` 앞에 4글자만 붙이면 N=400에서 recall이 **0.262 → 0.533**(두 배), N=24에서는
  **0.400 → 1.000**이 된다. R6(임계 0.05)은 "title을 바꿔도 안 움직이면 정렬 지배 진단이
  틀렸다"를 **반증하려고** 만든 기준인데, 실측 spread가 임계의 **7~12배**라 반증에 실패했다.
  → **"슬롯을 title 정렬이 정한다"는 살아남았다.**
- **E1의 P5 반증이 무엇이었는지 확정됐다**: E1에서 N≥200에 전멸(0.000)했던 `cwdIndependent`
  6개가 접두 하나로 **0.967**이 된다. "저장소 독립적 지식이 불리하다"가 아니라 **명명의 함수**였다.
- **내 가설 하나가 틀렸다**: N=400을 추가하니 평탄화가 아니라 **완만한 상승**이다
  (none 0.247 → 0.250 → 0.262, 세 조건 모두). R3은 200→400에 새 위반을 냈다. P3의 조작적
  정의(`|Δ| ≤ 0.05`)가 "평탄"과 "천천히 상승"을 구별하지 못한 것 — 내 기준의 결함이고 그대로 적었다.
- **측정 규율이 한 칸 나아갔다**: E1의 가장 큰 구멍이 "픽스처가 사전등록이 아니라 리포트
  커밋에서 처음 등장"이었다. E2는 픽스처 3개를 **사전등록 커밋에 동봉**하고, 스모크가
  `git log --diff-filter=A`로 "픽스처 커밋 < 리포트 커밋"을 **엄격 부등호**로 강제한다.
  등호를 허용하면 E1이 저지른 바로 그 사고에서 초록이 되므로 strictly-before여야 한다 —
  E1 파일 세트로 겨누면 위반 3건으로 FAIL함을 확인했다.
- 그 밖에 닫힌 것: filler title 중복(절단이 원인이라 풀을 키워선 못 막고, 절단 뒤 결정적
  유일 접미로 구성적 차단), 동결 예산 11B 오차(6,956 → **6,967**), R4 은퇴 후 R6으로 대체.
- **E1 재현은 불변**이다 — 확장 후에도 E1 경로가 `0.4000 / 0.2775 / 0.2450 / 0.2475`를 그대로 낸다.

**남은 작업은 [`docs/0-2_goals.md`](docs/0-2_goals.md)에 `/goal` 붙여넣기용으로 정리해 뒀다.**
E3(상승 원인·R3 재정의·실사용 title 분포), 축 D(유료·승인 필요), 축 B(표본 대기),
적대적 검증 goal, 재탕 금지 축까지. 전부 Claude Code만으로 처리된다.

---

**E1 벤치마크 — 게이트 recall@cap 측정 장치 구축·사전등록·발행 (브랜치 `bench/0-2-axis-a`, $0).**

`docs/0-2_benchmark.md`를 따라 계획서 **I6**를 실행했다. 산출물 4개가 전부 미생성이었으므로
하니스를 먼저 만들고, 사전등록서를 커밋한 뒤 측정하고 리포트를 발행했다.

- `lib/gate.mjs` 신설 — `bin/session-start.mjs`의 게이트 조립을 **순수 이동**하고 `stats`를
  선택 인자로 받아 제자리 변형한다. 훅 서브프로세스 출력과 모듈 출력이 **바이트 동일**함을
  4/4 확인했다. `bin/session-start.mjs`는 훅 껍데기만 남았고 호출부는 손대지 않았다.
- `test/gate-recall.mjs` 신설 — 결정적·유료 0 하니스. 유료 0은 선언이 아니라 **PATH 트랩**
  (`claude` 스텁을 PATH 앞에 심고 발동 여부를 기록)으로 실행 중에 증명한다.
- 커밋 순서: `b41f2ea`(사전등록) → `b842c24`(하니스·리포트) → `f37ff25`(README 8개 언어).
  G3-0f는 사전등록이 리포트보다 앞선 커밋일 것을 요구하고, `git log --diff-filter=A`로
  기계 검사된다.

**측정 결과** (4레벨 × 20시드 = 80샘플, 6.2초, $0.00):
`recall(24)=0.400 · recall(50)=0.277 · recall(100)=0.245 · recall(200)=0.248`.
**R2 발화**(recall(24) < 0.60), **R3 발화**(100→200 단조 위반 +0.0025, 해당 레벨 stdev 0.041).
R1·R4·R5 미발화. P1·P2·P5 반증. 사전등록한 규칙에 따라 **recall 절대값을 정책 근거로 쓰지
않는다** — 따라서 이 회차는 I2 착수 여부를 결정하지 않는다.

**실질적 발견**: 20문항 중 9개가 전 레벨 생존율 0, 3개가 전 레벨 1.0, 나머지 8개가 중간값이다.
concept 줄이 200~1,030B인데 index 예산은 6,956B라 **전체 taken이 8~11줄에서 소진**되고,
그 결과 카테고리당 슬롯이 1~3줄로 균등해진다. `references`는 어느 레벨에서도 taken이 1줄이라
거기 몰린 정답 8개 중 최대 1개만 산다.

그 슬롯을 정하는 것은 관련성이 아니라 **생성 시점의 정렬과 줄 길이**다 — type 섹션명을
대소문자 구분 정렬해 `# Subdirectories`가 항상 앞에 오고(`lib/index-gen.mjs:242`), 섹션 안은
frontmatter **`title` 사전순**이며(`:315` — 파일명이 아니다), 잔여 예산보다 긴 줄은 그
카테고리를 거기서 멈춘다(`lib/gate.mjs:122`). 현행 게이트에 cwd·최신성·질의어 참조는 0건이다.

⚠️ **초고에서 이걸 "파일명 사전순"이라고 썼다가 검증에서 틀린 것으로 확인해 고쳤다.** 정렬 키는
`title`이다. 다음 회차가 물어야 할 질문은 "N이 커지면 recall이 떨어지나"가 아니라
**"카테고리당 슬롯을 `title` 사전순과 줄 길이가 정하는 것이 맞나"**이다.

**계획서 사양 결함 2건을 실측으로 잡았다** — 둘 다 그대로 구현했으면 거짓 결론을 냈다:
- 생존 판정 정규식이 구 `- ` bullet 가정이라 v0.2 `* ` 포맷에서 **전 80샘플 합계 0**을 반환한다.
  그대로 썼으면 `recall=0`이 나오고 "게이트가 아무것도 못 싣는다"로 읽혔을 것이다.
- G3-0a의 동결 값(조립 9,218B / 절단 218B / 잔여 58B)이 **예산 회계 수정 이전** 상태라
  그대로 걸면 R5가 오발화한다. `live-shape-2026-07-26.json`에 재측정값을 못박았고, 기존
  `live-shape-2026-07-25.json`은 "수정 전에는 이랬다"의 기록으로 그대로 뒀다.

**축 A-2(중첩 깊이)도 N3·N4까지 마쳤다**: 평면 28 → 2단계 27 → 3단계 26 → 4단계 25로
깊이 1단계당 concept 줄 1개씩 감소하고, `taken_domain`이 네 조건 모두 0이라 사슬 펼치기
회귀는 없다. N3는 F 대비 -7.1%로 "20% 이상 감소"(상한 검토)도 "5% 미만"(공짜)도 아닌
**중간 판정** — 중첩은 공짜가 아니지만 깊이 상한을 둘 근거도 아니다. 원인은 구조 붕괴가
아니라 경로 문자열이 길어져 줄이 예산에서 밀리는 것이다.

**Codex 검증은 실행하지 못했다** — 워크스페이스 지출 한도 초과로 API가 거절했다
(`You hit your spend cap set by the owner of your workspace`). CLI·인증은 정상이다.
한도가 풀리면 `b842c24` 기준으로 다시 돌려야 한다.
...
```sh
node test/smoke.mjs
# 714 passed, 0 failed   (E1 벤치마크 패키지 + 적대적 검증 수정 반영 후. 700 -> 713(+13) ->
#                         714(하니스 사망 감지 단언 1건 추가). 순수 추가이며 기존 게이트 단언 23건 중
#                         수정된 것 0건 = 추출이 동작을 바꾸지 않았다는 증거.
#                         착수 시점 기준선은 303)

node test/gate-recall.mjs
# recall(24)=0.400±0.000 · recall(50)=0.277±0.038 · recall(100)=0.245±0.036 · recall(200)=0.2475±0.0413
# 각 n=20, R2·R3 발화, 캘리브레이션 8개 값 오차 0, paidCallTrapTripped: false, 6.2초, $0.00

node test/gate-recall.mjs --determinism-check
# index 구간 sha256 10/10 동일, 생존 집합 10/10 동일

node test/bench-nesting.mjs
# 평면 28 / 2단계 27 / 3단계 26 / 4단계 25 (taken_concept), taken_domain 전 조건 0
