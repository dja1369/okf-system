...
**축 E 벤치마크 — 게이트 효율: 동일 예산 전략 비교 (같은 브랜치 `bench/0-2-axis-a`, $0).**

E1~E3가 한 번도 묻지 않은 질문을 연다: **"이 인덱스 형식이 값을 하는가."** 세 회차 모두 OKF
자신의 입력만 흔들어서 비교 대상이 없었다. 이 회차는 같은 번들·같은 바이트 예산에서 인덱스
전략만 6종으로 갈아끼운다. 사전등록 `e028443`(하니스·쿼리 픽스처 동봉), 리포트 `4dbdecb`.

- **사전등록 가설 5개 중 2개 지지, 3개 반증.** 지지: 2026-07-17의 "카테고리 링크만 → 제목+설명"
  변경은 12/12 셀에서 옳았다(p<1e-4). 그동안 라이브 번들 아키텍처 문서에 **사례 1건·n=3**으로만
  적혀 있던 근거가 수치가 됐다. 반증: 설명을 떼면 **순서를 통제해도 12/12 셀에서 도달률이 오른다**;
  경로만 싣는 인덱스가 8/12 셀에서 우세; round-robin은 예산 2048에서 −0.050으로 해롭고
  9000에서 +0.017~+0.218로 이롭다(**출하 기본값에서는 옳고 그 아래에서는 틀리다**).
- **효율은 용량 × 정밀도로 분해된다.** OKF 줄은 실리기만 하면 거의 반드시 1위(정밀도 0.93~1.00)
  이고 병목은 용량이다 — 예산 9,000에 concept 줄이 12~14개만 들어간다. 설명이 줄 바이트의 82%다.
- **결론을 넘겨짚지 않았다.** 이 회차는 **찾기**만 쟀고 **답하기**는 못 잰다. 게이트 규칙 1의
  "설명이 답을 담으면 Read 없이 그 줄로 끝낸다" 경로는 유료 축이고 한 번도 안 돌렸다.
  그래서 "설명을 떼라"는 결론이 **아니다** — 가격표만 나왔다.
- **저자 편향을 하나 제거했다.** 쿼리를 손으로 쓰지 않고 정답 concept **본문**의 tf-idf 상위
  8항으로 기계 생성했다. 스모크가 생성기로 재생성해 바이트 동일인지 검사한다(손으로 고치면 잡힌다).
  다만 **정답 concept 목록 자체는 여전히 E1에서 승계한 내 선정**이다.
- **시드 수를 처음으로 검정력에서 정했다.** n=40, 시드 80% 일관 효과에 검정력 0.981(70%면 0.703).
  E3가 n=20을 관성으로 승계해 판정이 뒤집혔던 실패의 교정이다.
- **발행 전에 자기 결함 1건을 잡았다.** 첫 등록 실행에서 `paths` 도달률이 12셀 전부 0.000이었는데
  발견이 아니라 버그였다(채점기가 마크다운 링크 문법으로만 경로를 추출). 고치자 H5가 지지→반증으로
  뒤집혔다. 이제 스모크 단언이 고정하고 돌연변이 M1이 그 단언의 발화를 확인한다.
- **라이브 번들 실측(읽기 전용)**: 게이트가 예산의 **98.7%를 쓰고 번들의 53.8%만 보여준다.**
  합성 번들이 이 커버리지를 2.3%p 이내로 예측했다(합성 설계에 대한 외부 확인). 오버헤드 28.4% 중
  `log.md` tail 하나가 주입의 15.1%인데 **이 회차는 tail의 값어치를 재지 않았다.**
- README 8개 언어에 축 E 섹션을 **별도 섹션으로** 추가했다(E1~E3 통합 섹션과 중복 없음).

---

**직전 작업 — E3 벤치마크: 추세 판정 기준 재정의 (같은 브랜치, $0).**

E2가 남긴 네 가지를 닫는 회차. 사전등록 `3d9fda7`(픽스처 동봉), 리포트는 그 뒤 커밋.
**조건은 하나도 바꾸지 않았다** — 질문·레벨·시드·섭동·distractor·형상 전부 E2 승계.
조건을 건드리면 "E2의 15개 recall 셀이 그대로 재현되는가"라는 회귀 가드가 사라진다(15/15 일치).

- **이 회차는 자기 주 가설을 검정도 못 하고 끝났다.** E2가 "설명하라"고 넘긴 상승이
  방향 검정에서 **없다**: 12개 인접 레벨 쌍 중 `rising` 판정 **0개**. E2는 평균을 추세로 읽었다.
  H3(상승은 taken 성분이 만든다)은 검정 대상이 되지 못했고 R9는 **계산 불가**로 남았다.
- **옛 R3의 연속 발화는 결함의 증거가 아니었다.** 같은 300샘플에서 옛 R3은 발화하고,
  무결성을 직접 재는 **R3a는 발화하지 않는다.** 옛 R3은 문언이 "하니스 결함"인데 구현이
  임계 없는 평균 비교라 ±0.005 시드 잡음에 걸렸다. **임계를 완화한 것이 아니라** 탐지 대상을
  원래 문언대로 되돌린 것이다. E1·E2가 두 회차 연속 "발화했지만 폐기 안 함"이라는 자기모순
  상태로 발행된 이유가 이걸로 정리된다.
- **E2에서 내가 옛 기준의 결함에 붙인 이름이 위치를 잘못 짚었다.** 옛 기준이 감춘 것은
  상승이 아니라 **하락**이다 — none·front의 50→100이 p=0.0129/0.0352의 확립된 하락인데
  옛 기준은 평탄이라 불렀다. 상승 쪽에서는 옛·새 기준 판정이 전부 일치한다.
- **게이트 생존이 `rank < taken` 항등식으로 완전히 설명된다.** 300샘플 × 20문항 = **6,000건
  전부 일치**, 분해 잔차 ≤ 1.1e-16. 그래서 recall은 (rank, taken)의 완전한 함수이고 분해에
  근사가 없다. N≥100에서 rank 성분이 **정확히 0**으로 죽는 것은 바닥 효과(정답 rank 평균
  26.87 vs taken 10.50)이고, 남는 움직임은 전부 taken 성분이다. taken이 오르는 이유도 실측이다 —
  예산 고정인데 실린 줄 평균이 **702.1B → 598.3B**로 짧아진다. `front` N=24의 recall 1.000도
  같은 표로 설명된다(taken=24=후보 전체, 6개 카테고리 전부 소진 → 마커 전액 환급).
- **실사용 title 분포는 판정 불가**다. ASCII 선두 리프트 1.08×이지만 초기하 정확검정
  **p=0.667**(기대 11.12 vs 관측 12). 그리고 작게 나온 것을 "정렬은 무해하다"로 읽으면 안 된다 —
  **적재율이 65.4%**라 정렬이 아직 거의 안 물린다. 카테고리별로는 이미 갈린다
  (decisions·projects 1.000 / patterns 0.500 / references **0.429**). 적재율이 E2 결과와 E3
  결과를 잇는 매개다.
- **발행 전에 직접 잡은 결함 1건을 사전등록서 §9에 공개했다**: 배정도에서
  `0.25−0.20 = 0.04999…`, `0.20−0.15 = 0.05000…2`라 **같은 크기(문항 1개 이동)가 등가한계
  0.05의 반대편에 떨어져** 동일 CI에 다른 판정이 났다(flat vs indeterminate). recall이
  `맞은문항수/문항수`라 델타는 1/20의 정확한 배수여야 하므로 그 격자에 맞춰 양자화했다.
  **이 수정은 내 서사에 불리하다** — `indeterminate`가 사라졌고 그것이 살아 있었다면 새 기준의
  우위를 더 쉽게 보였을 것이다. 그래도 고친 이유는 같은 CI에 다른 판정을 주는 산술은 어느
  방향으로 유리하든 틀렸기 때문이다. 스모크가 이 결함을 회귀 고정한다.
- 라이브 번들 측정은 **읽기 전용**이고 산출물에 title·설명·파일명·링크가 한 글자도 없다
  (첫 글자 유니코드 분류 개수만). `raw/`는 `discoverConceptDirs`가 구조적으로 배제한다.
...
```sh
node test/smoke.mjs
# 735 passed, 0 failed   (착수 시점 실측 기준선 726에서 +9. 아래 E3 블록의 722는 E3 발행
#                         시점의 값이고, 그 뒤 E3 후속 정정에서 726까지 올라가 있었다.
#                         축 E가 더한 9건: 픽스처·하니스 선행성, 축소 실행 완주, 예산 준수,
#                         sel@1 <= reach 항등식, PATH 트랩 실측, 맨 경로 도달률 계측,
#                         okf/dump 전략 분리, 검색기 천장, 쿼리 픽스처 재생성 동일성)
#
# **돌연변이 6/6이 자기 가드를 죽였다** — 지워도 초록인 단언은 없다:
#   M1 경로 추출을 마크다운 전용으로 되돌림 -> FAIL      M4 쿼리 픽스처 손수정 -> FAIL
#   M2 fillToBudget이 예산 무시 -> FAIL (위반 15건)      M5 미도달을 1위로 셈 -> FAIL
#   M3 dump를 okf와 동일하게 -> FAIL                     M6 BM25 항상 0점 -> FAIL (천장 0.050)

node test/gate-efficiency.mjs
# 4 레벨 x 3 예산 x 40 시드, 약 30초. 가드 R1~R5 전부 통과(트랩 설치 true/발동 false,
# 예산 위반 0, 천장 0.950, 항등식 위반 0). 산출물이 현재 코드에서 바이트 단위 재현됨.
node test/gate-live-efficiency.mjs
# 라이브 번들 읽기 전용. concept 26개/108,431B, 주입 8,885B(예산의 98.7%), 커버리지 14/26=53.8%,
# 압축비 12.2x, 페이로드 71.6%. YAML `#` 절단 누수는 평문 스칼라 41개 중 0건(현재 시점).

# --- 직전 회차(E3) 기준선 ---
# 722 passed, 0 failed   (E1 -> E2 -> E3 누적. 700 -> 713(+13) -> 714(하니스 사망 감지) ->
#                         715(E2 픽스처 선행성) -> 722(+7: E3 픽스처 선행성, E3 축소 실행,
#                         생존 항등식 rank<taken 실측, 분해 잔차 0, 이산 격자 회귀 고정,
#                         방향 판정 네 값 폐쇄성, E1 경로가 미측정 필드를 null로 남기는지).
#                         전부 순수 추가이며 기존 게이트 단언 23건 중 수정된 것 0건
#                         = 추출이 동작을 바꾸지 않았다는 증거. 착수 시점 기준선은 303)

node test/gate-recall.mjs --e3 --perturb all
# E2의 15개 recall 셀 15/15 재현. rising 판정 0/12쌍. R2 발화, 옛 R3 발화하나 R3a 미발화.
# R8 미발화(6,000건 문항검사 전부 일치, 분해 잔차 1.1e-16). R9 계산 불가(귀속할 상승 없음).
# 300샘플 28.1초, paidCallTrapInstalled: true / Tripped: false, $0.00

node test/gate-title-distribution.mjs
# 라이브 번들 26 concept 중 17 적재(65.4%). ASCII 선두 리프트 1.08x (초기하 정확검정 p=0.667,
# 판정 불가). 읽기 전용이고 산출물에 title/설명/파일명/링크 없음.
