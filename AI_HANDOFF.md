...
축 D에는 **실행 전 사전 준비 가이드(§7-12)**와 **컨텍스트 예산 규칙(§7-13)**을 붙였다.
§7-9의 스크립트는 골격이라 그대로 돌지 않는다 — 스냅샷 동결·ON/PLACEBO 텍스트 생성·스키마와
헬퍼 채우기·격리 가드·사전등록서 커밋·비용 가드 6단계를 마쳐야 실행 가능하고, 각 단계에 판정
기준을 붙여 뒀다. 별도 세션에 그대로 넣을 goal 문장도 문서에 있다.
게이트는 SessionStart 훅이 넣는데 서브에이전트는 그 훅을 안 타므로, ON 조건은 스냅샷에
`bin/session-start.mjs`를 돌려 나온 `additionalContext`를 프롬프트에 붙여 만든다 — 바이트와
위치는 같지만 채널이 다르다는 한계를 사전등록서에 적어야 한다.
§7-13은 컨텍스트 소진을 사전등록 무력화 문제로 본다: 본실험 중반에 컨텍스트가 마르면 판정
기준을 기억하지 못한 채 결론을 쓰게 된다. 그래서 시끄러운 일(채굴·실행·채점·준비 1~4)은 전부
서브에이전트로 보내고 메인은 집계값만 들고 간다. 반환에는 반드시 schema를 걸고 전사·diff는
경로로만 받는다.
